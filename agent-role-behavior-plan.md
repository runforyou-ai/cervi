...
- 不建设经验沉淀、知识缺口清单与回答质量评测，由 [客服能力路线图](customer-service-roadmap.md) 规划。
- 不做客服指标与满意度，`outcome` 与 `outcome_reason` 只为后续统计留下事实；指标口径与满意度由客服能力路线图规划。
- 不做工作时间、排队位置与无人在线的差异化提示；按承接结果区分的转人工话术与 AI 解决后关单由客服能力路线图规划，启动时同步修订本方案。
