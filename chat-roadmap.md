...
Web 与桌面端群聊输入 `@` 后，候选首项提供「所有人」，支持键盘和鼠标选择。选择后移除本次 `@` 查询，在输入区显示可删除的独立标记，正文只保存实际消息内容；每份草稿最多一个，移除后取消提醒。手写或粘贴相同文字不形成结构化提醒，只有标记而没有正文时仍须填写文本。

发送接口直接使用已有 `GroupTextMessageInput` 传递 `mentionAll`，发送中、失败和已保存消息均按该字段在正文前展示当前界面语言的标记。失败恢复保留提醒状态；内容相同的重试沿用客户端消息编号，改变 `mentionAll` 后使用新编号。切换会话沿用线程卸载机制清理草稿，标记不跨会话保留。
