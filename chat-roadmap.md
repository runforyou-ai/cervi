...
聊天重构与实时能力按 [PR 实施清单](chat-realtime-pr-plan.md) 交付。PR01 以 `707fdff` 为代码核对基线，仅对齐本文及 Agent 路线图；下文定义统一消息顺序、同步、列表和置顶规则的交付目标，第 13 章保留各次交付时的历史边界。PR06 已统一各类会话的消息序号、分页和阅读水位，当前仍使用前台轮询；后续同步、列表活动时间和置顶规则不视为已完成。
...
Telegram 私聊文本入站保存平台消息身份与一层原文快照，以企业、渠道、机器人和聊天共同限定消息编号。原消息已在 Cervi 时关联本地引用；原消息或发送回执后到达时，在渠道身份锁保护的事务中补齐关联。未关联的引用展示发送者名称与原文，暂不提供原消息定位；AI 客服同样接收这一层引用上下文，原消息超出最近 100 条历史时仍保留原文。
