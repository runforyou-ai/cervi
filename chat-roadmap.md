...
2. G2 群资料与成员管理（已交付）：改名、增员、移除、退出、群主转让、参与者行复用，以及成员变化的系统事件和审计边界。
...
G2 支持群主修改名称、批量增员、移除成员和转让群主，普通成员可以退出；群主退出前必须转让，最后一位群主可以解散群聊并保留只读历史。成员重新加入时复用原 Participant 行，群名称和成员变化以类型化系统消息记录操作人与目标快照。
...
1. 按阶段 2B 的 G4 继续交付已读事实，并以独立 PR 接入群聊 `@Agent`。
