...
1. 建立会话头、当前 Conversation 时间线、禁用回复区和上下文栏组成的前端工作区骨架。
2. 企业成员文本回复；无负责人的排队批次在首次回复时由当前成员领取并激活。
3. 网站访客和成员页面使用 `after` 轮询新增文本消息，完成网站双方文本闭环。
4. 公开访客端点按第 4.5 节补齐防滥用限制：接口限速、消息长度与频率约束、未回复 Conversation 数量上限；先于外部渠道扩展交付。
5. ServiceSession 显式领取、挂起和结束命令。
6. 客户队列子筛选按状态和负责人落地，并与 URL 查询参数同步。
7. 根据真实产品需要增加网站渠道“只允许一个入站会话”的可选策略；默认多会话保持 Conversation 公开主键。
8. 未读、统一实时、文件、外部平台投递、转接、指标和满意度。
