...
网站 Messenger 展示客服回复携带的一层原文摘要，以「你 / 客服」表达发送方，不公开企业内部身份。访客本轮只查看摘要，不发起引用或跳转原文。网站 AI 客服读取历史时保留引用消息的一层结构化正文、发送者类型和来源编号，即使原文已超出最近 100 条上下文；引用不增加模型对话角色，已删除目标只保留编号与删除标记。
