...
## PR F5：双方 HTTP 增量补拉（成员侧已完成，访客侧待交付）

- 待交付：网站访客挂件使用现有公开历史接口的 `after` 游标轮询客服新消息。
- 已完成：成员打开的 Customer 和 Direct 时间线使用成员接口的 `after` 游标轮询新增消息；空 Direct 在取得第一条消息前使用无游标最近页。
- 已完成：消息路由不在活动工作区标签、页面不可见或窗口失焦时停止成员侧轮询，恢复后立即补拉；请求结果与本地发送结果按消息编号幂等合并，游标只向前推进。
