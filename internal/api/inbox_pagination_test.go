...
func (b *inboxPageBackend) LoadInbox(_ context.Context, _ appservice.RequestMeta, input appservice.LoadInboxInput) (appservice.Inbox, error) {
	b.input = input
	return appservice.Inbox{Conversations: []appservice.InboxConversation{}, HasMore: true, NextCursor: "next-page", UnreadCount: 80, AttentionUnreadCount: 70}, nil
}
...
func TestInboxPaginationHTTP(t *testing.T) {
	...
	for _, test := range []struct {
		...
		response.Body.Close()
		if response.StatusCode != http.StatusOK || err != nil || backend.input.Limit != test.limit || backend.input.Cursor != test.cursor || !page.HasMore || page.NextCursor != "next-page" || page.UnreadCount != 80 || page.AttentionUnreadCount != 70 {
