...
func (q *GetContactQuery) Execute(ctx context.Context, principal *servermodels.Principal, contactID string) (*ContactDetail, error) {
	contact, err := loadContact(ctx, q.db, principal.Organization.ID, contactID, false)
	if err != nil {
		return nil, err
	}
	var sourceChannel *SourceChannel
	if contact.SourceChannelID != nil {
		...
		if err := q.db.NewSelect().
			...
			Scan(ctx, candidate); errors.Is(err, sql.ErrNoRows) {
			sourceChannel = nil
		} else if err != nil {
