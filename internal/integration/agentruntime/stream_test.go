...
// TestStreamSnapshotApply 验证增量按序号应用，重复被忽略，缺口、换流和无法应用的操作不修改快照。
func TestStreamSnapshotApply(t *testing.T) {
	snapshot := StreamSnapshot{RunID: "run", StreamID: "stream"}
	first := StreamDelta{RunID: "run", StreamID: "stream", Sequence: 1, Operations: []StreamOperation{
	...
	if _, err := snapshot.Apply(StreamDelta{RunID: "run", StreamID: "stream", Sequence: 3}); !errors.Is(err, ErrStreamGap) {
		t.Fatalf("apply gap error = %v", err)
	}
	if _, err := snapshot.Apply(StreamDelta{RunID: "run", StreamID: "retry", Sequence: 2}); !errors.Is(err, ErrStreamMismatch) {
		t.Fatalf("apply other stream error = %v", err)
	}
	invalid := StreamDelta{RunID: "run", StreamID: "stream", Sequence: 2, Operations: []StreamOperation{
...
func TestMergeStreamOperations(t *testing.T) {
	...
	for i, operation := range operations {
		if _, err := stepwise.Apply(StreamDelta{RunID: "run", StreamID: "stream", Sequence: int64(i + 1), Operations: []StreamOperation{operation}}); err != nil {
			t.Fatal(err)
		}
	}
