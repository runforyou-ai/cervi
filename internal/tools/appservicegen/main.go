...
// appservice.Service 的纯委托方法、服务端 DirectBackend 的认证分发方法、
...
// delegation 描述一层委托方法的接收器、转发目标和认证注入方式。
...
func generateService(methods []method) []byte {
	...
	emitDelegations(builder, methods, delegation{receiver: "s *Service", target: "s.backend", skipManual: "service"})
	return []byte(builder.String())
}

// generateDirectBackend 生成服务端 Backend 的认证分发层。
//
// 除 auth=public 的方法外，每个方法先解析登录身份，再把身份交给 directOperations
// 中的业务实现；业务实现不重复处理认证。
