...
func TestGroupCursorPreservesSequence(t *testing.T) {
	...
	point := conversationaction.MessageCursorPoint{ConversationSequence: &sequence, ID: "0198ddf0-a234-7f01-8d99-e3e0af0f5f65"}
	cursor := encodeConversationMessageCursor(conversationID, point)
	decoded, valid := decodeConversationMessageCursor(cursor, conversationID)
	if !valid || decoded.ConversationSequence == nil || *decoded.ConversationSequence != sequence || decoded.ID != point.ID {
		t.Fatalf("decoded=%+v valid=%v", decoded, valid)
	}
	if got := conversationSequenceString(&sequence); got == nil || *got != "9007199254740993" {
