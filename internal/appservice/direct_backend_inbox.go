	...
	for _, summary := range summaries {
		...
		if summary.Customer != nil {
			...
			conversation.Customer = &CustomerInboxConversation{
				...
				Preview: summary.Customer.Preview, PreviewFormat: MessageBodyFormat(summary.Customer.PreviewFormat), LastMessageAt: summary.Customer.LastMessageAt,
		...
		if summary.Direct != nil {
			...
			conversation.Direct = &DirectInboxConversation{
				...
				Preview: summary.Direct.Preview, PreviewFormat: MessageBodyFormat(summary.Direct.PreviewFormat), LastMessageAt: summary.Direct.LastMessageAt, AgentRunStatus: agentRunStatus,
			}
		}
		if summary.Group != nil {
			conversation.Group = &GroupInboxConversation{
				...
				Status: ConversationStatus(summary.Group.Status), Preview: summary.Group.Preview, PreviewFormat: MessageBodyFormat(summary.Group.PreviewFormat),
