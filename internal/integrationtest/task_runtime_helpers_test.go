...
// newTestTasks 创建不连接 NATS 的任务运行时，并以空处理器注册客服处理周期分配、转人工承接、小结、待补知识起草与邮件通知任务，供业务事务投递。
func newTestTasks(db *bun.DB) *servertask.Runtime {
	...
	if err := tasks.Registry().RegisterJSON(customernotify.NotifyActionName, func(context.Context, customernotify.NotifyInput) error { return nil }); err != nil {
		panic(err)
	}
	return tasks
}
