...
func newNavigationFixture(t *testing.T) navigationFixture {
	...
	_, err = useraction.NewCreateUserAction(db, newTestTasks(db)).Execute(ctx, owner, useraction.CreateInput{HandlesCustomers: true, MaxServiceSessions: 10, DisplayName: "成员", Email: "member@navigation.test", Password: "password123", RoleID: owner.User.RoleID})
	if err != nil {
		t.Fatal(err)
	}
