...
func testAgentCustomerReplies(t *testing.T, db *bun.DB, identity *servermodels.Identity, providerID, modelID string) {
	...
	for _, scenario := range []struct{ earlierSession, deleted bool }{{false, false}, {false, true}, {true, false}, {true, true}} {
		t.Run(fmt.Sprintf("earlierSession=%t/deleted=%t", scenario.earlierSession, scenario.deleted), func(t *testing.T) {
			...
			if _, err := conversationaction.NewSendCustomerTextMessageAction(db, newTestTasks(db)).Execute(ctx, identity, conversationaction.CustomerTextMessageInput{ConversationID: original.Conversation.ID, ClientMessageID: uuid.NewV7().String(), Body: "针对早期问题的回答", ReplyToMessageID: original.Message.ID}); err != nil {
				t.Fatal(err)
			}
			if deleted {
