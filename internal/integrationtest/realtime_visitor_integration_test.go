...
func startVisitorRealtime(t *testing.T, f customerReadFixture, options gateway.Options) *visitorRealtimeHarness {
	...
	memberBackend := appservice.NewDirectBackend(f.db, domain.DeploymentModeSelfHosted, nil, serverfilecontent.S3Config{}, serverstorage.NewTenantResolver(f.db), nil, nil, nil, nil)
