...
func TestBackendMethodsRequireAuthentication(t *testing.T) {
	...
	backend := appservice.NewDirectBackend(db, domain.DeploymentModeSelfHosted, nil, serverfilecontent.S3Config{}, serverstorage.NewTenantResolver(db), nil, nil, nil, nil, nil, appservice.DeviceToolchainSources{})
