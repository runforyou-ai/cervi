...
func TestWebsiteCustomerIdentityHTTP(t *testing.T) {
	...
	application := appservice.New(appservice.NewDirectBackend(f.db, domain.DeploymentModeSelfHosted, nil, serverfilecontent.S3Config{}, serverstorage.NewTenantResolver(f.db), nil, nil, nil, nil, nil, appservice.DeviceToolchainSources{}))
