...
func testAgentReplyStopping(t *testing.T, db *bun.DB, identity *servermodels.Identity, agentID, agentIdentityID string, tasks *servertask.Runtime) {
	...
	executor := agentrunaction.NewExecuteAction(db, tasks, nil, testAttachmentReader(db))
	for range 2 {
	...
	if err := agentrunaction.NewExecuteAction(db, tasks, runtime, testAttachmentReader(db)).Execute(ctx, agentrunaction.RunInput{RunID: next.ID}); err != nil {
		t.Fatal(err)
	}
