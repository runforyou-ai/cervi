...
func TestCustomerReplyBoundaries(t *testing.T) {
	...
	if _, err := conversationaction.NewCloseServiceSessionAction(f.db, agentrunaction.NewExecuteAction(f.db, nil, nil, testAttachmentReader(f.db))).Execute(ctx, f.owner, f.conversationID); err != nil {
		t.Fatal(err)
	}
...
func TestCustomerReplyEarlierSession(t *testing.T) {
	...
	if _, err := conversationaction.NewClaimServiceSessionAction(f.db, agentrunaction.NewExecuteAction(f.db, nil, nil, testAttachmentReader(f.db))).Execute(ctx, f.owner, f.conversationID); err != nil {
		t.Fatal(err)
	}
	if _, err := conversationaction.NewCloseServiceSessionAction(f.db, agentrunaction.NewExecuteAction(f.db, nil, nil, testAttachmentReader(f.db))).Execute(ctx, f.owner, f.conversationID); err != nil {
		t.Fatal(err)
...
func TestWebsiteVisitorReplyBoundaries(t *testing.T) {
	...
	if _, err := conversationaction.NewClaimServiceSessionAction(f.db, agentrunaction.NewExecuteAction(f.db, nil, nil, testAttachmentReader(f.db))).Execute(ctx, f.owner, other.Conversation.ID); err != nil {
		t.Fatal(err)
	}
