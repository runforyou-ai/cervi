...
func testAgentFailureMessages(t *testing.T, db *bun.DB, identity *servermodels.Identity, tasks *servertask.Runtime, conversationID, firstRunID, secondRunID, nextRunID string) {
	...
	finalizer := agentrunaction.NewExecuteAction(db, tasks, nil, testAttachmentReader(db))
	...
	if err := agentrunaction.NewExecuteAction(db, tasks, runtime, testAttachmentReader(db)).Execute(ctx, agentrunaction.RunInput{RunID: nextRunID}); err != nil {
		t.Fatal(err)
	}
...
func testCustomerFailureMessage(t *testing.T, db *bun.DB, identity *servermodels.Identity, tasks *servertask.Runtime, agentID string) {
	...
	if err := agentrunaction.NewExecuteAction(db, tasks, nil, testAttachmentReader(db)).FinalizeFailure(ctx, agentrunaction.RunInput{RunID: run.ID}, errors.New("model failure details")); err != nil {
		t.Fatal(err)
	}
