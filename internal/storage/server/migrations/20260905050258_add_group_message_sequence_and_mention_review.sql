-- +goose Up
ALTER TABLE conversations ADD COLUMN last_group_message_sequence bigint NOT NULL DEFAULT 0;
ALTER TABLE messages ADD COLUMN group_message_sequence bigint;
ALTER TABLE conversation_user_states ADD COLUMN last_reviewed_mention_message_id uuid;

CREATE UNIQUE INDEX messages_group_message_sequence_unique
    ON messages (organization_id, conversation_id, group_message_sequence)
    WHERE group_message_sequence IS NOT NULL;
...
ALTER TABLE messages DROP COLUMN group_message_sequence;
ALTER TABLE conversations DROP COLUMN last_group_message_sequence;
