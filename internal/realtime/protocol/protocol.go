...
// CloseReason 定义服务端关闭帧中的原因；发送 RealtimeError 后以错误码作为原因关闭，接收方按未知原因处理未定义的值。
type CloseReason string

const (
	CloseSessionRevoked  CloseReason = "session_revoked"
	CloseSessionExpired  CloseReason = "session_expired"
	CloseIdleTimeout     CloseReason = "idle_timeout"
	CloseSlowConsumer    CloseReason = "slow_consumer"
	CloseServerGoingAway CloseReason = "server_going_away"
)
...
// ServerHello 返回连接编号和订阅安装后读取的同步探针值；变更通知可能先于本帧到达。
...
// RealtimeError 表示连接级错误，服务端随后关闭连接。
