...
本文是项目唯一的计划文档，只记录尚未交付的事项；已交付能力以代码为准。事项按领域分组，已确定范围的写成 PR 计划并保留设计细节，其余写粗略计划与启动条件。某个 PR 或计划完成后，直接删除本文对应内容，不另行记录交付情况。

## 总体顺序

1. 客服闭环：客服基础模型整理 → 转人工承接与分配 → 解决与关单、会话小结、运营报表与知识缺口、网站显式评价 → 访客与客户信息 → 第一批渠道与 AI 翻译 → 网站 Messenger 占位能力、AI 满意度与质检、联系人资料、常用语 → 第二批渠道、自定义 API 渠道、工单、开放接口与权限。
2. Agent：桌面端本地 Agent 运行（P4）。
3. 其余事项按各自的启动条件开启；安全、容量与性能在准备正式上线时集中处理。
...
- 先落地「设备注册与认证」中 `devices` 的注册、撤销、`work_seq` 和最近在线时间；设备撤销、登出、换服、切换账号和用户停用后，该设备不能领取 Run 或调用模型代理。
