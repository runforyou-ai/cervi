...
本文承接 [知识文档处理管线 Go 化与 Haystack 移除方案](knowledge-go-pipeline-plan.md) 第 10 节中「本地问答路在进入融合前按条目折叠」的未完成项，把问答知识库接入已有的分段、向量、词法与混合召回链路。Agent 运行期注入 `KnowledgeSearch` 另行提交，不在本文范围。
...
- 向量模型切换后的全库重建当前对文档库也未实现，问答库同样不在本轮处理。
...
索引与召回在同一个 PR 内交付，提交顺序为：状态类型共享改名、分段表来源化与条目索引列、问答处理任务与触发、问答召回与结果契约、前端列表状态与检索测试展示。文档库的行为在整个 PR 内保持不变。Agent 注入 `KnowledgeSearch` 在本 PR 之后单独提交，届时问答答案通过 `Record.Answer` 直接进入工具输出。
...
提交前执行 Pi CLI 审核并处理反馈，验证结束后清理本次启动的进程。
...
- 本次会话没有可用的浏览器自动化与桌面 MCP 工具，问答列表状态列、重试菜单和检索测试侧栏的界面效果未做可视化验证。
