...
当前实现自动解析、字符分段、分段向量化、分段查看和重新处理，不包含分段编辑、OCR 和检索召回。
...
上传完成后，Cervi 在激活原件的同一事务中保存文档和可靠任务。知识处理使用独立 Worker Pool；Worker 流式读取本地或对象存储原件，按文档快照解析知识库的向量模型凭据，交给 Python 解析、分段和向量化。Python 在临时目录处理文件，退出时清理。完整批次写入后，Go 核验当前任务、租约及分段数量，再发布批次。每个处理任务只执行一次，失败后直接记录失败状态与原因，不自动重试。文档列表在所有状态下均开放手动重试；先在三秒内检查服务连接，连接失败只保存失败状态，连接成功才使用知识库当前配置创建新任务。

## 数据归属与状态

- 原始文件仍由 Cervi 文件管理保存。
- `knowledge_documents` 只保存来源、处理状态、任务标识、分段与向量参数快照、已发布分段批次与数量，不保存分段正文。
- 分段正文和向量只保存在 `public.knowledge_segments` 的 `content`、`embedding` 和 `embedding_dimension`。Python 负责处理和查询，Cervi 迁移定义表结构；删除文档或知识库时，Go 在业务删除事务中同步删除分段。双方通过同一文档行锁协调删除和迟到写入。
...
分段完成后按知识库配置的向量模型逐批生成向量，每批 20 条输入，取供应商兼容接口允许的最小批量。模型凭据在执行任务时由 Go 从供应商记录解析为 OpenAI 兼容入口，随本次请求传递，不写入任务载荷。返回维度与知识库配置不一致时整批失败，不写入部分向量；调用失败保留上次成功结果，由人工重试。

向量维度限定为 384、512、768、1024、1536、2048、3072。`embedding` 列不带维度，混合维度按 `embedding_dimension` 区分；每个维度有一条 `hnsw ((embedding::halfvec(N)) halfvec_cosine_ops)` 部分索引，检索必须带维度谓词才能命中。halfvec 索引上限为 4000 维。
...
后续召回结果至少携带 `knowledgeBaseId / documentId / segmentBatchId / segmentId`。全文索引基于同一分段记录扩展，不能另建一份正文。当前表是 Python 管理的分段存储，尚未接入 `PgvectorDocumentStore` 或 Retriever；后续接入时需实现对应适配契约。
