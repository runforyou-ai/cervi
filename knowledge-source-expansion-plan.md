...
- 扩展集显式声明：StarterKit 之外单独引入标题、列表、引用、代码块、链接和表格（含表头、行、单元格）扩展，实现前确认所选版本对表格的往返序列化可用。
- Tiptap 官方文档说明 Markdown 表格每个单元格只允许一个子节点，编辑器按此限制单元格内容，粘贴内容按同一规则规范化。
- 禁用图片节点，粘贴与拖入的图片一并剥离；本轮在线文档不支持图片。
...
- 文档详情页对 `text` 与 `web` 展示正文，使用现有 Markdown 渲染；`file` 保持原件预览。
