...
**第 0 批 契约抽取。** `ExecuteAction` 拆成取指派、跑运行时、收尾三段；`ResolveAssignment` 落地，有效配置成为运行时入参、运行快照与设备执行指派的同一份结构，执行侧只注入供应商凭据。取指派这一段仍绑定访客提示、进程内取消登记与服务端运行时限，第 1 批上线设备路由时再分出与执行位置无关的部分。运行期与收尾路由随设备认证一起在第 1 批上线。验收：现有全部 Agent 集成测试保持通过，新增跨端有效配置一致性测试。
