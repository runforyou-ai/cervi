...
**第 0 批 契约抽取。** `ExecuteAction` 拆成取指派、跑运行时、收尾三段，服务端 Worker 与设备共用第一段和第三段；`resolveAssignment` 落地；运行期与收尾路由上线，先服务自身集成测试。验收：现有全部 Agent 集成测试不变绿，新增跨端有效配置一致性测试。

**第 1 批 设备与派发。** `devices`、`device_workspaces`、`conversation_device_bindings` 三张表与 `agent_runs` 列变更；设备注册、撤销、工作水位、事件流通知、领取、续租、租约过期失败；AI 单聊会话绑定。桌面端先跑返回固定文本的假运行时，把分布式正确性与运行时可移植性分开验证。验收：设备离线时 Run 排队不丢、上线后领取执行；杀掉桌面端后 Run 按租约失败且群聊轮转继续；停止后本机循环终止；设备撤销后不能领取。
