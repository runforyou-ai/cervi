...
- **运行时同一份。** 服务端与设备执行同一份 `internal/integration/agentruntime` 代码，有效配置（场景、依据策略、指令、模型参数、工具清单、MCP 与知识库绑定）由同一个解析器 `resolveAssignment` 产出。两端行为差异只允许来自工具清单。
...
**第 0 批 契约抽取。** `ExecuteAction` 拆成取指派、跑运行时、收尾三段，服务端 Worker 与设备共用第一段和第三段；`resolveAssignment` 落地，有效配置成为运行时入参、运行快照与设备执行指派的同一份结构，模型凭据只存在于执行侧注入的字段中。运行期与收尾路由随设备认证一起在第 1 批上线。验收：现有全部 Agent 集成测试保持通过，新增跨端有效配置一致性测试。

**第 1 批 设备与派发。** `devices`、`device_workspaces`、`conversation_device_bindings` 三张表与 `agent_runs` 列变更；设备注册、撤销、工作水位、事件流通知、领取、续租、租约过期失败；运行期与收尾路由连同设备头校验和租约校验一起上线；AI 单聊会话绑定。桌面端先跑返回固定文本的假运行时，把分布式正确性与运行时可移植性分开验证。验收：设备离线时 Run 排队不丢、上线后领取执行；杀掉桌面端后 Run 按租约失败且群聊轮转继续；停止后本机循环终止；设备撤销后不能领取。
