...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。验证闭环的主干已交付，当前补齐真实客户试用所需的完整度：网站 Messenger 占位能力 → 联系人资料与客户档案 → AI 质检、推断满意度与回答质量评测 → 真人运营指标；海外第一批渠道与上述事项并行推进 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。常用语按需启动。
...
网站 Messenger 按 Intercom 的结构搭建，以下内容当前为占位。
...
1. 助理使用企业 MCP 与群聊中的助理，见「P4：助理与桌面端本地运行」。
2. 任务规划、子 Agent 与后台任务，先在助理上提供。
3. 长期记忆。
4. 本机 Agent：助理经 ACP 交给主人电脑上的 Agent 完成，同上。
...
设备注册、派发领取、运行时编入、模型代理、助理身份、本机运行的知识检索、附件和过程流、本机文件读写与命令执行以及技能已上线，剩余按两批交付：企业 MCP 与群聊中的助理，本机 Agent。界面操作暂缓。
