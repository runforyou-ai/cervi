...
设备是一类受用户控制的 Agent Worker：服务端保留 Run 的全部业务事实、输入认领、结果事务、群聊轮转和企业模型凭据，设备借出本机文件和执行本机工具的进程。两端执行同一份 `agentruntime` 代码与同一个有效配置解析器，本机工具通过 Eino 的文件系统与 Shell 接口注入；本机 Agent 经 ACP 驱动，由其使用自身登录与订阅。每个会话在绑定电脑上有一个默认文件夹，由设备自行确定，服务端不保存本机路径。
...
- 本地运行是否把企业模型供应商凭据下发设备，或把本机路径写入服务端。
