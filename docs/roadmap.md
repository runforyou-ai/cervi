...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。AI 表现报表与知识回流 → AI 回复复核 → 离线回复通知、AI 翻译与多语言内置指令 → 网站 Messenger 占位能力、常用语、联系人资料、AI 满意度与质检、真人运营指标 → 海外第一批渠道 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。
...
| 事项 | 做法 |
|---|---|
| 解决情况 | 按小结的是否解决统计解决率，可按负责人类型、渠道和分类拆分；是否解决留空的周期单列，小结为无实质诉求的周期不计入 |
| 知识缺口聚类 | 由判断模型把统计范围内已结束会话中、原因为 `knowledge_gap` 与 `insufficient_evidence` 的转人工的客户提问按相近问题聚类，标注出现次数与示例会话；小结为无实质诉求的周期不计入 |
