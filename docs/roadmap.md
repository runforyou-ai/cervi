...
- AI 通过 MCP 工具查询业务系统，查询结果登记为回答依据；工具级「可作为回答依据」开关、默认值与通用判定（调用成功、未标记错误、结果非空）在启动时确定。
- 与「访客与客户信息」的登录用户识别同期交付：AI 以已识别的客户身份查询该客户自己的数据。
- 启动时确定接入配置、读取工具的依据判定，以及客服侧栏是否展示同一份查询结果；写操作随「P1.5：服务端副作用、审批和恢复」建设。
...
| 事项 | 优先级 | 做法 |
|---|---|---|
| 登录用户识别 | P1 | 企业网站用服务端签名把已登录用户身份传给 Messenger，关联到联系人；为 AI 读取客户档案和业务数据提供身份，与「业务系统接入」同期交付 |
