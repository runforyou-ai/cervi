...
「团队与队列是什么」「收件箱如何分视图」两件事在现有代码中各自不闭合，「转人工承接与分配」直接建在其上会继承这些问题。本节先把两者整理成一致的模型，完成后再启动分配。
...
| PR | 范围 |
|---|---|
| B 团队即队列 | 第 2 节全部，以及第 3 节的页签更名、队列下拉与团队标签 |
| C 收件箱视图口径 | 第 3 节其余：「全部」范围口径收敛、「待分配」状态限制、负责人头像 |
...
本方案让 AI 转人工和新进线的客户会话都有真人及时承接并回复。方案涉及接单与接待量、自动分配、积压补分配、未响应回收、队列等待提醒、工作时间和转人工话术。方案只写在现有转人工事务（`internal/actions/agentrun/customer_handoff.go`）之上的变化，并以「客服基础模型整理」交付后的模型为准：接待资格按身份上的接待开关判定，队列语义与收件箱队列筛选由该节提供。
