...
AI 转人工后必须有人及时承接并回复，否则 AI 接待链路不闭环。本节剩余工作时间、转人工话术、超时提醒与未响应回收。
...
方案写在现有分配能力之上：分配器（`internal/actions/serviceassignment`）按「先锁成员账号行、再锁会话」挑选接待量最少的可分配成员；转人工在交接事务内分配，入站、退回、转交给队列经分配任务分配，成员可接待量增加时经补分配任务补充。`service_sessions.awaiting_reply_since` 记录客户开始等待回复的时间，`assignee_assigned_at` 记录当前负责人接手时间；`service_attention` 实时帧已定向提醒成员，前端经新消息通知策略投递。
...
| 提醒通道 | 复用 `service_attention` 与新消息系统通知的投递链路和开关：全局消息通知开关、工作状态、应用可见性 |
...
- 没有定时扫描兜底：分配任务在没有可分配成员时结束，之后只能等成员补分配触发；删除团队后并入公共队列的周期也不会投递分配任务。
...
清空点与 `assignee_assigned_at`、`awaiting_reply_since` 的写入点一致：领取、接管、转交、转人工、自动分配、退回队列、客户新消息开始等待、回复结束等待、关闭与重开。
...
| 未响应回收 | 同上 | 同上 | 超过 `responseReclaimMinutes` 退回原队列，写 `service_session_returned`（原因 `response_timeout`），并在排除原负责人的前提下重新分配；原负责人随后补分配 |
...
同一扫描发现队列中有等待会话且存在可分配成员时投递分配任务，作为分配兜底。扫描粒度 30 秒，提醒和回收最多晚 30 秒左右。MVP 阶段不为扫描新增普通索引。

###### 4.3 提醒投递

`service_attention` 增加原因 `response_overdue`、`queue_waiting`、`returned`（发给被回收的原负责人），通知在事务提交后发布；前端按原因本地化文案，例如「客户等待回复已超过 5 分钟」，按 `service_attention:<周期编号>:<原因>` 去重。`service_attention` 是临时事件，客户端离线时丢失；会话的负责人和队列状态以收件箱为准。
...
- 模型主动转人工、依据不足、预算耗尽、无效输出、运行失败、AI 失去资格等全部交接路径统一使用上表，删除 `agent.customer_handoff_fallback` 与 `agent.customer_failure_fallback`。AI 失去资格的管理交接当前退回队列后经分配任务分配，改为在交接事务内分配，话术才能反映承接结果。
...
| 会话时间线 | 为 `service_session_returned` 补上未响应回收的原因文案 |
...
| PR | 范围 |
|---|---|
| B 工作时间与转人工话术 | 3.2 的两个设置键与「客服」设置页（本 PR 只含工作时间分组）；工作时间计算；第 5 节话术与 `handoff_to_human` 参数调整 |
| C 超时与提醒 | 3.1 的字段；「分配与提醒」设置分组；第 4 节扫描、未响应提醒与回收、队列等待提醒与分配兜底；`service_attention` 其余原因 |
...
设备注册、派发领取、运行时编入与模型代理已上线，剩余按五批交付：运行时补齐（知识检索、附件与本机流式渲染）、只读本机工具、写入与命令审批、群聊绑定、界面操作。
