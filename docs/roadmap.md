...
移动端与桌面端共用消息时间线、输入框、客服会话操作、客服侧栏、群设置和会话列表菜单。AI 员工、渠道、知识库、工具、模型服务、成员、角色、团队管理、AI 表现、企业与客服设置等管理页面只在 Web 与桌面端提供，移动端不设入口。以下 PR 补齐移动端日常使用的差距；外部联系人的删除、回收站与恢复、按渠道和联系方式筛选、排序只在桌面端提供。
