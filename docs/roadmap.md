...
本文是计划总览，只记录尚未交付的事项；已交付能力以代码为准。事项按领域分组，已确定范围的写成 PR 计划并保留设计细节，其余写粗略计划与启动条件。篇幅较大的实施方案拆成 `docs/` 下的独立文档，本文只留范围与入口。某个 PR 或计划完成后，直接删除本文对应内容和已实现的独立方案文档，不另行记录交付情况。

本文与 `docs/` 下的方案文档是主要参考，不构成强制约束。实际开发中经常出现新的想法和变动，出现更合适的做法时按新做法实施，并同步更新对应文档；文档里已经写下的设计本身不是必须照做的理由。

## 总体顺序

1. 客服闭环：转人工承接与分配 → 解决与关单、会话小结、运营报表与知识缺口、网站显式评价 → 访客与客户信息 → 第一批渠道与 AI 翻译 → 网站 Messenger 占位能力、AI 满意度与质检、联系人资料、常用语 → 第二批渠道、自定义 API 渠道、工单、开放接口与权限。
2. 聊天：会话输入状态 PR A、PR B。
3. Agent：桌面端本地 Agent 运行（P4）。
4. 其余事项按各自的启动条件开启；安全、容量与性能在准备正式上线时集中处理。
...
- 不建设通用规则引擎；只提供确定性规则，即「转人工承接与分配」未响应回收和「解决与关单」超时关单。
...
| 事项 | 做法 |
|---|---|
| 接单开关 | 复用成员工作状态：只有「工作中」的客服参与自动分配。自动分配只看工作状态和当前接待量（本人负责的开放客服周期数），每人最大接待量默认 10。离开「工作中」后，已负责的会话仍由本人处理 |
| 团队队列 | 在开启接单的团队成员中按接待量分配；无人可分配时留在团队队列 |
| 公共队列 | 在全部开启接单的客服中按接待量分配；无人可分配时留在公共队列 |
| 指定成员 | 指定成员不在「工作中」时改走渠道备用路由，备用路由不可用时进入公共队列 |
| 未响应回收 | 负责人超时未回复时提醒；超过上限退回原去向的队列（有团队时为团队队列，否则为公共队列）并重新分配 |
| 积压分配 | 客服切换为「工作中」或接待量下降时，按等待时间从其所在团队队列和公共队列补充分配 |
...
| 移动端 | 移动端可切换工作状态，分配到本人时经本地通知提醒；远程推送另立事项 |

转人工话术与工作时间提示替换 AI 客服现有的对客通知规则。

#### 实施方案（PR 计划）

本方案让 AI 转人工和新进线的客户会话都有真人及时承接并回复。方案涉及接单与接待量、自动分配、积压补分配、未响应回收、队列等待提醒、工作时间和转人工话术。方案只写在现有转人工事务（`internal/actions/agentrun/customer_handoff.go`）之上的变化。
...
| 在线与离线 | 本方案不建设，随「网站 Messenger」的在线标识一起建设。超时按负责人是否回复判断，与在线状态无关 |
...
AI 负责的周期不适用这些规则，其超时跟进由「解决与关单」定义。计时按自然时间，不扣除非工作时间。重新分配无人可接时周期留在队列，由队列等待提醒接续。
...
入站新周期路由到队列时不发送对客话术；访客侧的等待说明由「网站 Messenger」的回复时间文案承担。
...
| B 工作时间与转人工话术 | 3.3 的两个设置键与「客服」设置页（本 PR 只含工作时间分组）；工作时间计算；第 6 节话术与 `handoff_to_human` 参数调整 |
...
- 成员在线与离线状态、Messenger 在线标识：见「网站 Messenger」。
- 移动端远程推送（APNs、FCM）。
- 现有新消息通知对全部客户会话生效的范围收窄。
- AI 负责周期的超时跟进与关单：见「解决与关单」。
- 按技能或咨询分类分配：见「会话小结与分类」的意图路由。

### 解决与关单（P1）

超时跟进与超时关单只作用于仍由 AI 负责、且最后一条对客消息来自 AI 的客服周期，包括普通回答、追问和解决确认。客服周期进入队列或由真人负责后，AI 不再发言，客户失联由客服关单，负责人未回复按「转人工承接与分配」未响应回收处理。
...
| 回复时间 | 固定显示「我们通常会尽快回复」 | 渠道由 AI 员工首接待时表达 AI 可立即回答；真人接待或转人工后按「运营报表与知识缺口」真人首响统计和「转人工承接与分配」工作时间说明预计等待时间 |
...
- 工单实现状态、优先级、负责人、团队、会话消息关联、内部评论、对外回复、SLA 和自动化，由 AI 从客户会话生成工单并填写字段。
...
| 批次 | 渠道 | 要点 |
|---|---|---|
| 第二批 | 自定义 API 渠道 | 渠道自带入站接口与出站 Webhook，企业自行接入任意平台；与「工单、开放接口与权限」开放 API 相互独立 |
...
转人工话术与工作时间、AI 解决后关单与超时关单、运营报表、知识缺口、回答质量评测、经验沉淀与满意度见「客服运营与渠道」。
...
用户坐在自己电脑前，让 AI 员工在本机读写代码、执行命令和操作界面，覆盖 AI 单聊和只有群主一个真人的群聊。

设备是一类受用户控制的 Agent Worker：服务端保留 Run 的全部业务事实、输入认领、结果事务、群聊轮转和模型凭据，设备借出一个工作区和一个执行本机工具的进程。两端执行同一份 `agentruntime` 代码与同一个有效配置解析器，本机工具通过 Eino 的文件系统与 Shell 接口注入，审批在设备本地同步完成。会话按「设备加工作区」绑定，工作区真实路径只存在设备本地。

按七批交付：契约抽取、设备与派发、运行时编入、只读本机工具、写入与命令审批、群聊绑定、界面操作。

数据结构、契约、租约语义、工具与审批规则、安全边界、批次验收见 `docs/desktop-local-agent.md`。
...
执行命令、读写代码和操作界面属于高频本机工具循环，采用桌面端本地运行 Agent 的方案，见「P4：桌面端本地 Agent 运行」。Capability Executor 承担服务端 Agent 对设备的低频类型化调用，随 P2 在真实场景出现后落地。
...
| 本机执行（AI 员工讨论群主电脑中的文件） | 群会话绑定群主设备与工作区，群内只有群主一个真人；建立运行时写入执行设备并由设备领取，见 P4 | 轮转规则、点名接力、轮次语义与执行互斥 |
...
8. 本地运行与服务端运行共用 Agent 身份、Revision、Lane、Input、Run 和最终消息幂等，不创建第二套身份、表或空字段。
9. Device Capability Gateway 复用 Realtime Gateway 的事件流、认证、JSON 事件、通知发布、NATS 和背压，不建设第二套实时基础设施。
10. 本地运行的 Run 由服务端建立并指定执行设备，设备领取整个 Run 在本机执行；输入认领、最终消息、Run 终态、水位推进和群聊轮转在服务端事务提交，模型供应商凭据只保存在服务端。
...
- 查询性能索引：迁移当前只保留主键和用于业务约束、幂等及并发正确性的唯一索引，上线前统一评估并补齐。
- 密钥加密存储与接口响应脱敏。
