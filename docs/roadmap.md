...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。离线回复通知、访客端多语言 → 网站 Messenger 占位能力、常用语、联系人资料、AI 满意度与质检、真人运营指标 → 海外第一批渠道 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。
2. 工作台信息架构与服务对象扩展：海外首发闭环的 P1 事项交付后启动服务会话泛化与 Cervi 单聊员工服务台（IT 先行），其后接入内部 IM 机器人；伙伴服务台在出现真实客户时启动。见 [工作台信息架构与服务对象扩展方案](workbench-ia.md)。
3. Agent：以 Eino 为参照先接入行业标准能力（Skills、上下文摘要、任务规划、子 Agent、长期记忆等）并推进助理与桌面端本地运行（P4），约束层逐步完善；与客服闭环并行推进，不占用同一批业务改动。
...
移动端与桌面端共用消息时间线、输入框、客服会话操作、客服侧栏、群设置和会话列表菜单。AI 员工、渠道、知识库、工具、模型服务、成员、角色、企业与客服设置等管理页面只在 Web 与桌面端提供，移动端不设入口。以下 PR 补齐移动端日常使用的差距，按顺序推进。

**PR 1：移动端小项补齐。**

- 收件箱筛选面板增加「重置筛选」；切换待处理类型时与桌面端一致清空队列筛选。
- 图片预览提供保存，走与非图片附件相同的下载链路。
- 「我的」增加设备列表，复用桌面端设备列表页，可撤销设备。
- 新建群聊支持设置群头像和群描述，与桌面端建群对话框字段一致。
- 「我的」页面底部显示应用版本号。

**PR 2：移动端外部联系人编辑。**

- 外部联系人详情支持编辑名称、阶段、邮箱、电话和备注，列表支持新建联系人和按阶段筛选。
- 删除、回收站与恢复、按渠道和联系方式筛选、排序留在桌面端。
- 验收：客服在移动端客户会话资料页或通讯录修改阶段与备注后，桌面端联系人详情与客户会话资料同步更新。

移动端气泡保持不可局部选择文本，长按菜单承担引用与复制；聊天列表以类型筛选代替桌面端的分节折叠，置顶以菜单上移下移代替拖动。这些是移动端交互取舍，不列入对齐。
...
- 访客可直接浏览的帮助中心文章、比 AI 生成更稳定的常用语等确定性能力与 AI 互补保留。
- 优先补齐 AI 解决问题所需的底座：客户身份、业务数据查询与历史上下文；其次是 AI 无法替代的环节：转人工后的承接、会话结束口径和离线后的触达。
- 不建设通用规则引擎；只提供确定性规则，即已上线的负责人未响应提醒与回收，以及 AI 负责时的超时跟进与超时关单。
- 运营闭环以 AI 表现为中心：AI 解决了多少、为什么转人工、缺哪些资料，并把缺口回流到知识库。
- 首发市场为海外，用网站与 Telegram 验证 Agent First 闭环：AI 查询业务数据并答复、转人工承接、解决口径和知识回流。其他渠道不作为闭环验证的前置条件。

### 业务系统接入

AI 以网站登录用户的身份通过 MCP 查询工具取得业务数据并据此答复，写操作随「P1.5：服务端副作用、审批和恢复」建设。

| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| 业务面板 | 客服侧栏「业务」页签只列出 AI 客服的查询记录 | 出现客服需要在业务系统中查看或办理客户业务的反馈。侧栏以 iframe 嵌入业务系统自有页面，Cervi 以客户身份密钥签发短期上下文令牌（客户 `sub`、邮箱、会话编号、客服成员，独立 `iss`／`aud`），业务系统验签后展示并操作该客户数据；客户未验证时不签发，移动端在应用内浏览器打开。不建设插件运行时与模板协议 |
| 向访客展示业务数据卡片 | AI 以纯文本答复查询结果 | 出现需要结构化展示订单、物流等信息的需求；启动时评估 MCP Apps 标准的成熟度与 Go SDK 支持，不自定义模板协议 |
| 匿名身份并入登录用户 | 客户登录前以匿名身份发起的会话不并入，匿名期间开启的客服周期在登录后仍为匿名 | 出现客户反馈登录后找不到会话 |
| 其他渠道的客户身份验证 | 只有网站 Messenger 支持签名身份 | 出现需求时按渠道单独设计，复用企业级客户身份密钥与企业用户编号 |
| 客户身份密钥轮换重叠期 | 重新生成密钥立即生效 | 出现网站无法接受身份中断的反馈 |
| 客户身份密钥的查看权限 | 任何成员都能查看密钥 | 随角色权限统一收口 |
| 访客地区解析 | 访客地区只取部署配置的可信代理国家头，不内置 IP 地理库 | 出现无可信代理国家头的部署需要访客地区 |

### AI 表现报表与知识回流

| 事项 | 做法 |
|---|---|
| 知识缺口聚类 | 启动条件：待补知识清单条目多到逐条浏览难以发现共性问题。由判断模型把统计范围内的待补知识按相近问题聚类，标注出现次数与示例会话 |

AI 客服运行结果是报表与知识回流的事实来源。报表对企业成员全员可见，数据范围随权限统一建设。待补知识只回收知识不足或缺少依据的转人工，以及 AI 关闭后被评价未解决或判断可能答错的周期；客户要求转人工、投诉与需人工判断多与个案或流程相关，不起草问答。
...
| AI 推断满意度 | P2 | 由判断模型推断满意度，覆盖全部客服周期；解决情况沿用小结的是否解决 |
| AI 质检 | P2 | 真人与 AI 客服的会话都检查，由判断模型标记答错、态度问题、应转人工未转等情况，其中 AI 答错的周期并入待补知识清单 |
| 真人运营指标 | P2 | 转人工后真人首响、AI 处理时长（进线到转人工或 AI 关单）、人工处理时长（接手到关单）、客服接待量、各渠道会话量与咨询分类分布。现有 `first_response_at` 含 AI 回复，只作为整体首响 |

### 访客与客户信息

| 事项 | 优先级 | 做法 |
|---|---|---|
| 离线回复通知 | P1 | 见下方「离线回复通知（P1）」 |
| 信息收集 | — | 由 AI 通过 `ask_customer` 在对话中收集，不做开聊前表单 |
| 联系人资料 | P2 | AI 从对话中抽取姓名、电话、订单号等更新联系人资料；支持联系人标签与自定义字段 |
| 重复联系人 | P2 | AI 提示疑似重复，客服确认后手动合并 |
| 黑名单 | 按需 | 出现骚扰或滥用时启动。客服屏蔽联系人或渠道身份，屏蔽后不再创建客服周期；启动时确定进行中客服周期的处理、屏蔽后入站消息是否回复，以及网站 Messenger 是否仍可打开 |

### 离线回复通知（P1）

转人工后访客往往已离开，真人客服稍后的回复通过邮件送达访客，匿名访客点开邮件回到原会话继续对话。只做网站渠道：Telegram 等即时通讯渠道由平台自身推送。

| 部分 | 做法 |
|---|---|
| 邮件发送 | 部署级 `email.smtp` 配置（主机、端口、用户名、密码、发件地址与发件名称），支持 `SMTP_*` 环境变量覆盖并纳入 `--check-config` 校验，使用成熟的 SMTP 库发信。未配置时整个能力关闭：不询问邮箱，也不发送通知。与邮件渠道相互独立 |
| 收集邮箱 | 转人工不因收集邮箱等待。网站渠道转人工进入队列（未直接分配给真人）、联系人没有邮箱且部署已配置发信时，转人工话术末尾追加一句请访客留下邮箱；工作时间内外、模型主动转人工与运行失败等系统强制转人工一律适用。其后在该客服周期由真人接手前，访客消息中恰好包含一个合法邮箱时由服务端写入联系人，并回复一条确认话术。写入规则与签名身份邮箱一致：同一联系人已有该邮箱不重复插入，没有主邮箱时设为主邮箱，已有主邮箱不覆盖 |
| 访客已读位置 | 客户会话记录客户最后读到的消息与时间。挂件展开或独立聊天页打开、且浏览器标签处于前台时，Messenger 把已读位置推到最新的对客消息，前台期间新到的消息同样计入；只显示启动器不算已读。服务端不维护访客在线状态 |
| 触发与合并 | 真人成员在网站渠道发出对客消息（文本或附件）时，在同一事务按客户会话幂等登记通知任务：同一会话同时只有一个待执行任务，从第一条未通知的回复起算 3 分钟，其间的后续回复并入该任务。任务只纳入不早于首条触发回复、晚于客户已读位置和上次通知位置的真人对客消息，为空即结束。AI 消息与内部备注不触发 |
| 发送与水位 | 发信在事务外执行，SMTP 成功后推进会话的已通知位置并追加内部系统事件「已通过邮件通知客户」（仅客服可见，不计入未读回复，不触发通知）。发信失败不推进水位，按任务重试；联系人没有邮箱或发信配置已关闭时直接结束，不重试也不写事件 |
| 邮件内容 | 语言沿用转人工话术的规则：客户语言，未知时取渠道默认接待语言，发信时不另行翻译。主题为「<企业名称> 回复了你的消息」；正文逐条列出回复的发送者姓名与已发给客户的消息正文（翻译发送时为译文），附件列出文件名；同时提供 HTML 与纯文本两部分，页脚注明直接回复邮件不会送达客服 |
| 回到会话 | 匿名访客的邮件带「继续对话」按钮，链接为企业 `access_host` 下的 `/chat/<channelID>?resume=<令牌>`。令牌由服务端签发，绑定渠道身份与客户会话，30 天内可重复使用。页面加载后由脚本调用换取接口，换得与现有渠道访客 Cookie 相同的凭据并打开该会话；同一浏览器已有的其他匿名身份被替换。客服周期已关闭时仍打开该会话，令牌失效时打开渠道首页。签名身份访客的邮件不带按钮，提示登录企业网站后在客服窗口继续对话 |

### 访客端多语言（P1）

客服侧的消息翻译已按客户语言工作，访客直接看到的界面与系统话术仍只有中文与英文。

| 事项 | 当前行为 | 做法 |
|---|---|---|
| 访客挂件界面 | 按钮、评价邀请、会话结束等界面文案按 Accept-Language 在中文与英文间选择 | 扩展到首发市场的常用语言 |
| 转人工与非工作时间话术 | 按客户语言在中文与英文间选择，其他语言使用渠道默认接待语言 | 随挂件界面一并补齐，话术在模型调用之外生成，不在会话锁内翻译 |

### 客服翻译

| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| 内部备注翻译 | 只翻译对客消息，内部备注按原文展示 | 出现不同语言的客服之间通过内部备注协作的需求 |
| Copilot 填入回复的语言 | Copilot 给出的对客回复按其自身判断的语言书写 | 出现客服填入 Copilot 回复后需要再改写语言的反馈 |
...
| 占位内容 | 当前行为 | 做法 |
|---|---|---|
| 帮助中心 | 首页「探索帮助」卡片和「帮助」页签（文章合集、搜索、文章详情、仍需帮助入口）使用固定示例文案 | 文章与合集从知识库发布，访客可直接浏览；访客搜索时由 AI 直接回答并附相关文章，未解决时进入对话。只发布标记为公开的内容，知识库不默认公开；启动时确定 AI 回答是否创建客服周期、是否计入「AI 表现报表与知识回流」 |
...
| 回复时间 | 固定显示「我们通常会尽快回复」 | 渠道由 AI 员工首接待时表达 AI 可立即回答；真人接待或转人工后按「满意度与质检」中真人运营指标的真人首响统计和企业客服工作时间说明预计等待时间 |
| 语音消息 | 录音只在预览模式可用 | 访客发送语音消息，AI 转写后进入客服与 AI 上下文 |

### 常用语（P2）

- 个人与企业两级常用语，纯文本，在对客输入区快速检索并插入草稿，作为比 AI 生成更稳定的固定答复。
- 不做变量、多级分类、附件和使用统计。
...
首发渠道为已接入的网站与 Telegram。首发市场为海外，海外批次先于国内批次启动。
...
- 收件箱按成员能否接待服务会话显示，不按团队判断；以员工为服务对象的 AI 员工在通讯录中以服务台列出；涉及个人信息的 AI 员工可设为只在单聊受理。

| 阶段 | 内容 | 前置 |
|---|---|---|
| B 服务会话与 Cervi 单聊服务台 | 服务对象、服务会话泛化、转人工去向调整、可见性按角色；成员单聊以员工为服务对象的 AI 员工产生服务周期并可转人工；发起人服务状态、通讯录服务台、IT 知识问答与转人工承接 | 海外首发闭环的 P1 事项交付 |
| C 员工 IM 接入 | 收件箱按接待资格显示、群话题服务周期、员工目录同步、身份范围注入、内部 IM 机器人渠道，其后 MCP 查询、身份系统执行与主管审批；验收时判断是否需要工单 | B；审批依赖 P1.5 |
| D 伙伴服务台 | 伙伴组织与成员、身份绑定、按伙伴路由、业务系统查询类工具 | C 中的身份范围注入；需要开放查询接口的真实客户 |

与其他事项的关系：渠道按「渠道接入」的内部 IM 机器人批次交付；伙伴进入内部会话协作见「受管外部协作」；工具审批依赖「P1.5：服务端副作用、审批和恢复」；数据范围随「开放接口与权限」的角色权限一并收口。

## 4. AI 客服与客服 AI 辅助

业务系统接入、AI 表现报表、知识回流与满意度见「客服运营与渠道」。
...
| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| 客户档案注入 | 客服上下文注入当前周期的对客消息与同一客户最近几条历史小结，不注入客户档案 | 随联系人资料落地 |
...
Agent 能力先紧跟行业标准：以当前依赖的 Eino 为参照，把 `adk/middlewares` 已提供的标准能力尽快接入，AI 员工与助理共用同一份 `agentruntime`，一次接入两边可用；依赖本机文件与命令的能力只在助理上提供。审批、策略、审计、副作用状态机等约束见「逐步完善的约束」，能力接入不以它们为前置条件。

### 能力接入顺序

1. 助理的 Skills，见「P4：助理与桌面端本地运行」。
2. 联网搜索与网页读取，AI 员工与助理同时接入。
3. 助理使用企业 MCP 与群聊中的助理，同上。
4. 上下文摘要与中断修补，AI 员工与助理同时接入。
5. 任务规划、子 Agent 与后台任务，先在助理上提供。
6. 长期记忆。
7. 本机 Agent：助理经 ACP 交给主人电脑上的 Agent 完成，同上。

工具检索与 AI 员工的 Skills 在出现真实场景时接入。

### Skills

- 技能采用 Agent Skills 标准：一个技能是含 `SKILL.md`（名称、简介与正文）和参考文件、脚本的文件夹，网上现成的技能可以直接使用。
- 安装技能就是 Agent 把技能包下载到技能目录，由成员在对话中让助理完成；Cervi 不建设技能管理平台。
- 运行时接入 Eino `adk/middlewares/skill`：模型只看到技能名称与简介，需要时加载正文与参考文件，脚本经命令执行运行。
- 首批只在助理上提供：助理具备本机文件与命令执行，能自行安装技能并运行脚本。AI 员工在服务端没有文件系统与命令执行，出现真实场景时再确定其技能的提供方式。
- 内置技能只收录与 Cervi 自身业务相关的技能，具体内容在出现明确需要时确定。

### 联网搜索与网页读取

- AI 员工与助理都可以搜索网络并读取网页内容，用于查资料和查找可安装的技能。
- 具体实现在实施时调研社区方案后选定；需要外部服务凭据的，由企业在设置中配置。

### 上下文摘要与中断修补

- 接入 `summarization`，上下文接近模型窗口时摘要压缩，与现有 `reduction` 的工具结果裁剪配合。
- 接入 `patchtoolcalls`，修补中断后遗留的悬空工具调用。
- 面向客户的 AI 员工只接入本批能力；规划、子 Agent 与后台任务会增加回复延迟与费用，客服场景出现明确需要时按配置开启。

### 任务规划、子 Agent 与后台任务

- 接入 `plantask` 提供任务创建、更新与列表工具，运行过程按任务展示进度。
- 接入 `subagent`，把子任务委派给独立上下文的子 Agent；技能的 `fork` 模式复用同一机制。
- 接入 `backgroundtask`：长时间命令与子 Agent 在后台运行，生命周期绑定所在 Run。设备长任务由客户端工作项承担，服务端唤醒仍由 `task_runs` 承担。

### 长期记忆

- 接入 `automemory`，记忆内容由 Cervi 持久化并可由用户查看与删除，检索复用本地知识库的转换与向量管线。
- 记忆按归属隔离：助理的记忆只属于其主人；AI 员工的记忆按服务对象归属，与 [客户身份与业务系统接入方案](customer-identity-business-data.md) 的客户身份对齐，一位客户的记忆不出现在其他客户的对话中。
- 记忆的来源、写入时机与遗忘交互随本批确定。
...
成员在自己的电脑上创建助理，助理在主人的电脑上查阅和整理资料、修改文档与表格、改代码，面向普通成员；默认使用企业模型服务，电脑上已装好支持 ACP 的本机 Agent（如 WorkBuddy、Codex、Claude Code）的成员可以改由其完成。助理只进入主人所在的单聊和群聊，群成员都可以让它在主人的电脑上工作；客服会话只使用 AI 员工。AI 员工只在服务端执行，助理只在主人绑定的电脑上执行，执行位置由归属决定。

设备是一类受用户控制的 Agent Worker：服务端保留 Run 的全部业务事实、输入认领、结果事务、群聊轮转和企业模型凭据，设备借出本机文件和执行本机工具的进程。两端执行同一份 `agentruntime` 代码与同一个有效配置解析器，本机工具通过 Eino 的文件系统与 Shell 接口注入；本机 Agent 经 ACP 驱动，由其使用自身登录与订阅。每个会话在绑定电脑上有一个默认文件夹，由设备自行确定，服务端不保存其位置。

设备注册、派发领取、运行时编入、模型代理、助理身份、本机运行的知识检索、附件和过程流以及本机文件读写与命令执行已上线，剩余按三批交付：Skills，企业 MCP 与群聊中的助理，本机 Agent。界面操作暂缓。

产品定义、交互、数据模型与批次验收见 [助理与桌面端本地运行实施方案](desktop-local-agent.md)。

「运行策略与审计」与「P1.5：服务端副作用、审批和恢复」两节是 Agent 的约束层（harness），统称「逐步完善的约束」：能力接入不以它们为前置条件，按各自的启动条件逐步补齐，安全相关部分在准备正式上线时集中处理。正式发布前删除开发期 `calculator` 工具。

### 运行策略与审计

- 补齐 Policy、State、Step 和 Tool Invocation。
- 服务端类型化 Tool 通过 `AgentExecutionContext` 调用 Action。
- 支持通用自动响应策略、费用和失败审计。
...
使用 `agent_run_steps` 记录模型、工具、审批或交接等有界语义步骤：
...
| 本机执行（助理在主人电脑上处理群内请求） | 助理只由主人加入其所在的群；建立运行时写入助理绑定的电脑并由设备领取，见 P4 | 轮转规则、点名接力、轮次语义与执行互斥 |
...
- 本地运行是否把企业模型供应商凭据下发设备，或让服务端依赖本机路径定位文件。
...
- 助理的本机安全能力：改动前确认、群成员的权限区分、可访问文件范围与设备密钥签名。
