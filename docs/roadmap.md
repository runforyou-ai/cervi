...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。验证闭环的主干已交付，当前补齐真实客户试用所需的完整度：网站 Messenger 帮助中心 → 联系人资料与客户档案 → AI 质检、推断满意度与回答质量评测 → 真人运营指标；海外第一批渠道与上述事项并行推进 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。常用语与 Messenger 语音消息按需启动。
2. 工作台信息架构与服务对象扩展：推进 Cervi 单聊员工服务台（IT 先行），其后接入内部 IM 机器人；伙伴服务台在出现真实客户时启动。见 [工作台信息架构与服务对象扩展方案](workbench-ia.md)。
3. Agent：以 Eino 为参照先接入行业标准能力（Skills、任务规划、子 Agent、长期记忆等）并推进助理与桌面端本地运行（P4），约束层逐步完善；与客服闭环并行推进，不占用同一批业务改动。
...
移动端与桌面端共用消息时间线、输入框、客服会话操作、客服侧栏、群设置、会话列表菜单和助理编辑表单。AI 员工、渠道、知识库、工具、模型服务、成员、角色、团队管理、AI 表现、企业与客服设置等管理页面只在 Web 与桌面端提供，移动端不设入口。外部联系人的删除、回收站与恢复、按渠道和联系方式筛选、排序，以及新建助理与换电脑只在桌面端提供。

移动端气泡保持不可局部选择文本，长按菜单承担引用与复制；聊天列表以类型筛选代替桌面端的分节折叠，置顶排序使用菜单上移下移或排序模式的拖动手柄。这些是移动端交互取舍，不列入对齐。
...
| 回答质量评测 | P2 | 从真实会话与待补知识中沉淀带标准答案的评测集，离线评估 AI 客服是否答对、是否应转人工未转，作为调整提示词、更换模型和知识库召回时的回归基准；评测结果决定是否增加「依据范围校验」 |
| AI 质检 | P2 | 真人与 AI 客服的会话都检查，由判断模型标记答错、态度问题、应转人工未转等情况，其中 AI 答错的周期并入待补知识清单 |
| 真人运营指标 | P2 | 交付后网站 Messenger 由团队或公共队列接待时，工作时间内的回复预期改按真人首响统计说明预计等待时长（当前只说明尽快回复）。指标包括转人工后真人首响、AI 处理时长（进线到转人工或 AI 关单）、人工处理时长（接手到关单）、客服接待量、各渠道会话量与咨询分类分布。现有 `first_response_at` 含 AI 回复，只作为整体首响 |
...
| 联系人资料与客户档案 | P2 | AI 从对话中抽取姓名、电话、订单号等更新联系人资料；支持联系人标签与自定义字段。AI 客服与 Copilot 的上下文同时注入客户档案（当前只注入本周期对客消息与最近几条历史小结），资料的写入与使用在同一计划内交付 |
| 重复联系人 | P2 | AI 提示疑似重复，客服确认后手动合并 |
| 黑名单 | 按需 | 出现骚扰或滥用时启动。客服屏蔽联系人或渠道身份，屏蔽后不再创建客服周期；启动时确定进行中客服周期的处理、屏蔽后入站消息是否回复，以及网站 Messenger 是否仍可打开 |

### 访客端多语言

对客语言为中文、英文和印地语，覆盖访客挂件界面、转人工与非工作时间话术和回复通知邮件。其他首发市场语言在出现对应市场的客户时补充：在 `domain.CustomerLocales` 登记语言，并在 `internal/i18n/locales/customer` 下补齐对客词条。
...
### 网站 Messenger 帮助中心（P2）

网站 Messenger 按 Intercom 的结构搭建。首页「探索帮助」卡片和「帮助」页签（文章合集、搜索、文章详情、仍需帮助入口）当前使用固定示例文案，本计划把它们换成企业的真实内容。

**发布范围**

- 网站渠道设置中增加「帮助中心」，选择要发布的知识库，可多选。选中即对该渠道的所有访客公开，选择处写明这一影响；未选择任何知识库时，Messenger 不显示「探索帮助」卡片和「帮助」页签。
- 发布范围只由渠道决定，知识库本身不设公开标记：同一企业的多个网站可以发布不同的知识库，内部资料放在不被任何渠道选中的知识库中。
- 一个知识库对应一个文章合集，合集名称与说明取知识库的名称与说明；合集内的文章是该知识库中的文本文档。文件和网页导入的文档用于 AI 检索，不作为文章发布。
- 网站渠道设置保存已选知识库列表；知识库删除时从各渠道的选择中移除。

**访客浏览与搜索**

- 访客在帮助页签浏览合集与文章，文章正文按知识库文本文档的原内容渲染。
- 访客搜索时，检索范围只包含该渠道已发布的文章，按知识库的全文与向量融合召回。渠道首接待为 AI 员工时，用该 AI 员工的模型根据召回内容单次生成回答，不走工具循环，回答下附相关文章；首接待不是 AI 员工时只列出相关文章。
- 搜索与 AI 回答不创建会话和客服周期，不计入「AI 表现报表与知识回流」。
- 访客点击「仍需帮助」进入新对话，搜索词预填到输入框，由访客确认后发送。

**不在本计划内**

| 事项 | 启动条件 |
|---|---|
| 独立的公开帮助中心网页（自有域名、搜索引擎收录） | 出现企业需要在 Messenger 之外对外提供帮助文档 |
| 文章多语言版本 | 出现同一网站面向多个语言市场的客户 |
| 文章手动排序、置顶与浏览统计 | 出现运营需要调整文章展示顺序或评估文章效果 |
| 帮助中心自助解决率 | 报表需要衡量访客未进入对话即解决的比例 |

### 网站 Messenger 语音消息（按需）

录音当前只在预览模式可用，真实访客不可见。启动条件：出现访客需要发送语音的渠道场景。做法：访客发送语音消息，AI 转写后进入客服与 AI 上下文。

### 常用语（按需）

启动条件：出现客服需要固定话术、AI 写回复无法满足的反馈。
...
| 阶段 | 内容 | 前置 |
|---|---|---|
| B 服务会话与 Cervi 单聊服务台 | 可见性按角色；成员单聊以员工为服务对象的 AI 员工按其服务对象产生服务周期，并按「办不了交给谁」转人工；发起人服务状态、通讯录服务台、IT 知识问答与转人工承接 | — |
| C 员工 IM 接入 | 收件箱按接待资格显示、群话题服务周期与只在单聊受理、渠道服务对象、员工目录同步、身份范围注入、内部 IM 机器人渠道，其后 MCP 查询、身份系统执行与主管审批；验收时判断是否需要工单 | B；审批依赖 P1.5 |
...
| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| 客户历史作为依据 | `search_customer_history` 检索同一客户已关闭周期的对客消息，结果不计入依据 | 出现 AI 因历史沟通无法作为依据而转人工的反馈；需要先确定历史中的承诺与当前业务状态冲突时的处理 |
| 分段读回拼接 | 模型分多次读回同一转存结果时，各次读回都不计入依据 | 出现多行大结果的依据来源，且模型常按分段读回 |
| 依据范围校验 | 当前边界内任一依据来源有效即放行正文，不校验正文是否忠实于资料 | 「满意度与质检」中的回答质量评测交付后，按评测结果决定是否增加运行期校验 |
...
1. 任务规划、子 Agent 与后台任务，先在助理上提供。
2. 长期记忆。
3. 本机 Agent：助理经 ACP 交给主人电脑上的 Agent 完成，见「P4：助理与桌面端本地运行」。

工具检索与 AI 员工的 Skills 在出现真实场景时接入。

### Skills

- 技能由成员在对话中让助理安装，Cervi 不建设技能管理平台。
- AI 员工在服务端没有文件系统与命令执行，出现真实场景时再确定其技能的提供方式。
- 内置技能只收录与 Cervi 自身业务相关的技能，具体内容在出现明确需要时确定。
- 项目级技能与 `fork` 模式见 [助理与桌面端本地运行实施方案](desktop-local-agent.md)「技能」。
...
- 面向客户的 AI 员工暂不接入本批能力：规划、子 Agent 与后台任务会增加回复延迟与费用，客服场景出现明确需要时按配置开启。
...
设备注册、派发领取、运行时编入、模型代理、助理身份、群聊中的助理、本机运行的知识检索、附件和过程流、本机文件读写与命令执行、技能以及企业与本地 MCP 已上线，剩余群内助理展示与本机 Agent。界面操作暂缓。
...
| 跨 Run 复用上下文摘要 | 长会话出现因按窗口预算截取会话历史而丢失早期信息的反馈 |
...
| PDF 表格与多栏版面 | 按文字层逐行提取并按字号识别标题，表格按行输出为空格分隔的文字，多栏版面按内容流顺序输出 | 出现以表格或多栏排版 PDF 为主的资料且召回质量明显受影响；需要先评估版面分析方案 |
| 音频与视频转写 | 上传只接受文本、网页、PDF 与 Office 格式，音视频原件无法导入 | 出现音视频资料入库需求；实现时使用自托管 ASR |
| YouTube 来源 | 不支持视频站点链接 | 与站点抓取一并评估；需要先确定字幕获取方式与出网边界 |
...
- 帮助中心搜索的 AI 回答按渠道限速，控制匿名访客产生的模型费用。
...
- 服务端抓取网页（知识库网页导入与 AI 员工的网页读取）拒绝内网与本机地址，重定向后的地址同样校验。
