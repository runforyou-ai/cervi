...
1. 客服闭环：转人工承接与分配（超时提醒与回收） → 解决与关单、会话小结、运营报表与知识缺口、网站显式评价 → 访客与客户信息 → 第一批渠道与 AI 翻译 → 网站 Messenger 占位能力、AI 满意度与质检、联系人资料、常用语 → 第二批渠道、自定义 API 渠道、工单、开放接口与权限。
...
AI 转人工后必须有人及时承接并回复，否则 AI 接待链路不闭环。本节剩余超时提醒与未响应回收。
...
方案写在现有分配能力之上：分配器（`internal/actions/serviceassignment`）按「先锁成员账号行、再锁会话」挑选接待量最少的可分配成员；转人工在交接事务内分配；AI 员工失去资格退回队列时经转人工承接任务（`agent.customer_returned_handoff`）分配并按承接结果通知客户，任务耗尽重试后改投分配任务；入站、真人负责人退回、转交给队列经分配任务分配，成员可接待量增加时经补分配任务补充。`service_sessions.awaiting_reply_since` 记录客户开始等待回复的时间，`assignee_assigned_at` 记录当前负责人接手时间；`service_attention` 实时帧已定向提醒成员，前端经新消息通知策略投递。企业客服设置存放在 `customer_service_settings`（每个企业一行，缺失时按默认值处理），在系统设置「客服」页维护。

##### 1. 已确定的决策

| 事项 | 决策 |
|---|---|
| 超时阈值 | 企业级设置，作为 `customer_service_settings` 的字段，在「客服」设置页新增「分配与提醒」分组维护 |
...
- 可靠任务支持延迟执行（`AvailableAt`）和 Cron 调度，没有撤销接口，也没有企业维度。
...
`customer_service_settings` 增加：

| 字段 | 默认值 | 含义 |
|---|---|---|
| `response_reminder_minutes` | 5 | 负责人超过该时长未回复时提醒负责人 |
| `response_reclaim_minutes` | 15 | 负责人超过该时长未回复时退回队列并重新分配；必须大于提醒时长 |
| `queue_reminder_minutes` | 5 | 会话在队列中等待超过该时长时提醒对应成员 |
...
| 规则 | 适用周期 | 起算 | 动作 |
|---|---|---|---|
| 未响应提醒 | 开放、负责人为真人、`awaiting_reply_since` 非空 | `max(awaiting_reply_since, assignee_assigned_at)` | 超过 `response_reminder_minutes` 提醒负责人一次 |
| 未响应回收 | 同上 | 同上 | 超过 `response_reclaim_minutes` 退回原队列，写 `service_session_returned`（原因 `response_timeout`），并在排除原负责人的前提下重新分配；原负责人随后补分配 |
| 队列等待提醒 | 开放、负责人为空、`awaiting_reply_since` 非空 | `awaiting_reply_since` | 超过 `queue_reminder_minutes` 提醒一次：团队队列提醒该团队中工作中的客服，公共队列提醒全部工作中的客服 |
...
| 位置 | 变化 |
|---|---|
| 系统设置 | 「客服」页增加「分配与提醒」页签维护三个时长，与「工作时间」使用与 URL 同步的页签切换 |
...
- 团队级工作时间：企业出现不同时区或不同班次的团队时增加。团队选择沿用企业设置或单独维护，转人工话术按会话进入的队列取用（团队队列取团队设置，公共队列取企业设置）；Messenger 的回复时间文案按渠道首选路由对应的队列推导。工作时间计算只接收一份设置，与设置来源无关。
