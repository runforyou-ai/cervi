...
本文是项目的主计划文档，只记录尚未交付的事项；已交付能力以代码为准。事项按领域分组，已确定范围的写成 PR 计划并保留设计细节，其余写粗略计划与启动条件。某个 PR 或计划完成后，直接删除本文对应内容，不另行记录交付情况。

规模较大、需要独立评审的方案另开文档：本文保留范围、关键决定、阶段划分和与其他事项的关系，完整契约、数据模型与验收留在该文档。当前有 [官方托管服务建设方案](saas-hosting-plan.md)。
...
4. 官方托管服务：按 [官方托管服务建设方案](saas-hosting-plan.md) 的阶段推进，与客服闭环并行，不占用同一批业务改动。
5. 其余事项按各自的启动条件开启；安全、容量与性能在准备正式上线时集中处理。
...
「谁接待客户」「团队与队列是什么」「收件箱如何分视图」三件事在现有代码中各自不闭合，「转人工承接与分配」直接建在其上会继承这些问题。本节先把三者整理成一致的模型，完成后再启动分配。

#### 1. 现状问题

- 接待资格的唯一开关是角色种类 `roles.kind = customer_service`，一个身份只有一个角色，管理员和自定义角色无法接待客户。
- 能被派单的人和能接单的人不是同一批：只有客服角色能被路由、被转交、出现在同事筛选里，但任何有效真人都能查看全部客户会话并领取、接管、回复、关闭、重开，授权不看角色。
- 权限码只保存不校验，自定义角色勾选权限不会获得任何客服语义（既有约定，本节不改）。
- 真人与 AI 的失效处理不一致：AI 停用或失去客服角色会交接其开放周期，真人停用或改角色既不交接周期，改角色也不重置引用他的渠道路由。
- 工作状态不参与任何接待判定（由「转人工承接与分配」接入，本节不改）。
- `service_sessions.team_id` 写入后无人读取：收件箱「排队中」只按负责人为空判断，团队队列与公共队列混为一谈；领取、转交、重开不维护该字段，删除团队也不清理它。
- 转交只能转给个人，没有转给团队或退回队列的操作。
- 路由到团队只检查团队存在，空团队或团队内无客服时仍然命中，吞掉本应生效的备用路由。
- 收件箱「全部」范围按「本人负责或参与过」筛选，参与关系不随客服周期清除，历史周期回复过的会话长期滞留。

#### 2. 接待资格

- `organization_identities` 增加 `handles_customers boolean NOT NULL DEFAULT false`，真人与 AI 员工共用。
- 角色不再承载接待语义：内置「客服」角色保留为权限预设，`RoleKindCustomerService` 不参与任何接待判定；`behavior.go` 的 AI 基线与 `BehaviorProfile` 改按该开关选择客服基线。基线改判与开关同一 PR 交付，否则开启接待的 AI 员工会按成员基线运行。
- 有效接待身份 = 开关打开 + 账号有效（AI 员工仍要求托管执行与当前 schema 版本）。`identity/customer_service.go` 与 `agentrun/customer_schedule.go` 中重复的判定合并为一处。
- 授权收敛：只有有效接待身份可以领取、接管、对客回复、关闭、重开客服周期，也只有他们可以作为渠道路由目标与转交目标。对客回复的隐式领取（`send_customer_text_message.go` 的 `applyMemberReplySessionPlan`）与界面按钮使用同一判定。未开启的成员仍可查看客户会话、写内部备注、被 @，作为协作者参与。
- 开关下发：`CurrentUser` 与成员、AI 员工的读取结构都带上该字段；开关变化推进身份资料版本（`identity/profile_version.go`），本人客户端据此立即生效。
- 失效处理对真人与 AI 统一，在停用账号或关闭开关的同一事务内依次完成：取消该身份在途的 Agent 运行并结算其输入队列（沿用 `HandOffAgentServiceSessions` 的做法）；把其负责的开放周期退回原队列（负责人清空，`team_id` 保持），逐个写入 `service_session_returned` 事件，原因 `assignee_unavailable`；重置引用该身份的渠道路由为公共队列。
- `service_session_returned` 事件与其时间线渲染在本节交付，「转人工承接与分配」的未响应回收直接复用。
- 退回队列后的对客通知沿用现有的 `agent.customer_failure_fallback`，本节不改对客文案；到「转人工承接与分配」再统一替换为按承接结果的话术。
- 界面：成员创建表单与成员详情、AI 员工表单增加「接待客户」开关，默认关闭；关闭时隐藏与接待相关的字段（最大接待量随「转人工承接与分配」加入）。开关与角色相互独立，选择内置「客服」角色不会自动打开开关，表单中两者分开展示。工作状态菜单中「工作中」的说明只对开启接待的成员提示会被分配新客户会话。

#### 3. 团队即队列

- `service_sessions.team_id` 表示会话所属队列：为空是公共队列，否则是该团队的队列。领取、接管、转交给个人、重开都不改变队列。
- 转交目标扩展为个人、团队、公共队列：输入由单一的 `AssigneeIdentityID` 改为 `ServiceSessionTarget`（`member | team | public_queue`）。转交给团队或公共队列时清空负责人并写入对应 `team_id`，等同于退回队列；转交给个人不改 `team_id`。
- 转交界面在现有的负责人选择中分组展示：同事（含 AI 员工）、团队、退回公共队列。转给团队时与路由采用同一条规则。
- 路由到团队与转交给团队都要求团队内至少有一名开启接待的有效**真人**成员，否则视为不可用，继续解析备用路由与公共队列。AI 员工不承接队列，纯 AI 团队不算可用。
- 删除团队时，在同一事务内把该团队的全部客服周期（含已关闭）并入公共队列（`team_id` 置空），避免已关闭周期重开后指向已删除的团队。
- 团队沿用通讯录团队，不新增客服组概念：一人可属多个团队，AI 员工也可入团队。

#### 4. 收件箱视图

- 页签保持四项：「待分配▾ / 我负责的 / @我的 / 同事▾」。现有的「排队中」更名为「待分配」，中英文文案键一并修改。AI 员工负责的会话仍并入「同事」，下拉中保留 AI 标记，不单独成一个视图。
- 「待分配」增加队列下拉：全部队列、公共队列、各团队（本人所在团队排在前面）。选中「全部队列」时，列表项在名称后显示所属团队的小字标签，公共队列不加标签；选中具体队列时不显示标签。
...
| PR | 范围 |
|---|---|
| A 接待资格开关 | 第 2 节全部：字段、判定合并、授权收敛与开关下发、路由与转交目标改判、失效处理与 `service_session_returned`、AI 基线改判、成员与 AI 员工表单开关 |
| B 团队即队列 | 第 3 节全部，以及第 4 节的页签更名、队列下拉与团队标签 |
| C 收件箱视图口径 | 第 4 节其余：「全部」范围口径收敛、「待分配」状态限制、负责人头像 |

#### 6. 验收

- 关闭接待开关的成员看不到领取、接管、回复、关闭入口，仍可写内部备注并被 @；打开开关后立即可用。
- 管理员或自定义角色打开开关后可被渠道路由、被转交，并出现在同事筛选中。
- 真人停用或关闭开关后，其开放周期退回原队列，引用他的渠道路由重置为公共队列；AI 员工同样。
- 关闭 AI 员工的接待开关时，其在途运行被取消，负责的周期回到原队列并写入 `service_session_returned`。
- 转交到团队或公共队列后负责人清空、队列正确，会话出现在「待分配」对应队列中；转交给没有接待真人的团队被拒绝。
- 删除团队后其开放周期出现在公共队列；该团队的已关闭周期重开后进入公共队列。
- 首选路由指向空团队或只有 AI 员工的团队时，按备用路由、公共队列依次降级。
- 「全部」范围不再出现仅在历史周期参与过、当前由他人负责的会话。

#### 7. 对「转人工承接与分配」的影响

- 可分配成员的判定由「角色为客服」改为「接待开关打开」。
- 团队队列、退回原队列的语义与 `service_session_returned` 事件直接沿用本节，P0 不再自行定义。
- P0 界面清单中的收件箱队列筛选与团队标识由本节交付，P0 只新增最大接待量、客服设置页与提醒。
...
本方案让 AI 转人工和新进线的客户会话都有真人及时承接并回复。方案涉及接单与接待量、自动分配、积压补分配、未响应回收、队列等待提醒、工作时间和转人工话术。方案只写在现有转人工事务（`internal/actions/agentrun/customer_handoff.go`）之上的变化，并以「客服基础模型整理」交付后的模型为准：接待资格按成员的接待开关判定，队列语义与收件箱队列筛选由该节提供。
...
- 收件箱「待分配」视图按当前周期无负责人筛选（`load_inbox.go` 的 `filterCustomerInbox`），队列区分与团队标签由「客服基础模型整理」交付。
- 停用或关闭接待开关后退回队列的处理由「客服基础模型整理」统一；本方案只负责退回后的重新分配。
...
- 真人身份，账号有效，开启接待（沿用「客服基础模型整理」的有效接待身份判定）；
- 工作状态为「工作中」；
- 当前接待量小于最大接待量；
- 团队队列时是该团队成员，公共队列时是企业内任一开启接待的真人成员。

候选按当前接待量升序、`last_service_assigned_at` 升序（空值优先）、身份编号排序，取第一位。AI 员工不参与队列分配，其工作状态也不参与路由。未开启接待的成员既不参与分配，也无法领取或回复（见「客服基础模型整理」）。
...
团队队列中无人可分配时，周期保持 `team_id = 团队`、负责人为空；公共队列时二者都为空。自动分配成功后 `team_id` 保留为该周期所属的队列，用于回收时退回原队列。领取、接管、转交给个人不改变 `team_id`；转交给团队或公共队列按「客服基础模型整理」改变队列。
...
| 真人停用、真人关闭接待开关 | 退回原队列由「客服基础模型整理」完成，本方案在该事务内为退回的周期逐个投递分配任务 |
...
| `service_session_returned` | 未响应回收、停用或关闭接待开关退回队列 | 原负责人、退回的队列、原因 `response_timeout` 或 `assignee_unavailable` |
...
| 工作状态菜单（桌面端导航、移动端「我的」） | 保持现有三项；「工作中」的帮助文案对开启接待的成员说明会被分配新的客户会话（文案随「客服基础模型整理」交付） |
...
- 接待开关、队列语义、收件箱队列筛选与视图口径：见「客服基础模型整理」。
...
官方 SaaS 独立仓库、独立数据库，承担官方账号、订阅计费和运营后台，通过 cervi-server 的 `/operator/v1` 运营 API 开通和管理企业。cervi-server 在同一进程启动两个 HTTP 监听地址：企业业务监听地址经公网企业域名入口对外，运营监听地址只在受控网络提供运营路由。契约上运营接口使用独立的 `OperatorBackend` 和独立生成目标，`Backend` 仍是企业业务调用的唯一契约源。
