...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。访客端多语言 → 网站 Messenger 占位能力、常用语、联系人资料、AI 满意度与质检、真人运营指标 → 海外第一批渠道 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。
...
| 转人工与非工作时间话术、回复通知邮件 | 按客户语言在中文与英文间选择，其他语言使用渠道默认接待语言 | 随挂件界面一并补齐，话术在模型调用之外生成，不在会话锁内翻译 |
