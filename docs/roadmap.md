...
规模较大、需要独立评审的方案另开文档：本文保留范围、关键决定、阶段划分和与其他事项的关系，完整契约、数据模型与验收留在该文档。当前有 [官方托管服务建设方案](saas-hosting-plan.md) 和 [桌面端本地 Agent 运行实施方案](desktop-local-agent.md)。

本文与 `docs/` 下的方案文档是主要参考，不构成强制约束。实际开发中经常出现新的想法和变动，出现更合适的做法时按新做法实施，并同步更新对应文档；文档里已经写下的设计本身不是必须照做的理由。

## 总体顺序

1. 客服闭环：收件箱视图口径 → 转人工承接与分配 → 解决与关单、会话小结、运营报表与知识缺口、网站显式评价 → 访客与客户信息 → 第一批渠道与 AI 翻译 → 网站 Messenger 占位能力、AI 满意度与质检、联系人资料、常用语 → 第二批渠道、自定义 API 渠道、工单、开放接口与权限。
2. Agent：桌面端本地 Agent 运行（P4）。
3. 官方托管服务：按 [官方托管服务建设方案](saas-hosting-plan.md) 的阶段推进，与客服闭环并行，不占用同一批业务改动。
...
收件箱「全部」范围与「待分配」视图的口径与客服周期不一致，「转人工承接与分配」直接建在其上会继承这些问题。本节先收敛口径，完成后再启动分配。

#### 1. 现状问题

- 权限码只保存不校验，自定义角色勾选权限不会获得任何客服语义（既有约定，本节不改）。
- 工作状态不参与任何接待判定（由「转人工承接与分配」接入，本节不改）。
- 收件箱「全部」范围按「本人负责或参与过」筛选，参与关系不随客服周期清除，历史周期回复过的会话长期滞留。

#### 2. 做法

- 状态筛选仍在漏斗浮层里；选中「待分配」时状态强制为「未关闭」，浮层中的「已关闭」置灰，切到其他视图后恢复可选。
- 「全部」范围的客户会话口径收敛为：本人负责，或在**当前**客服周期内参与（回复、内部备注、被 @）；历史周期遗留的参与关系不再带入。
- 「同事」与「全部」视图的列表项显示负责人小头像。

#### 3. 验收

- 「全部」范围不再出现仅在历史周期参与过、当前由他人负责的会话。
- 「待分配」视图中状态筛选固定为未关闭，切换到其他视图后恢复可选。

#### 4. 对「转人工承接与分配」的影响

- 队列语义与收件箱队列筛选已经交付，P0 不再自行定义，只新增最大接待量、客服设置页与提醒。
...
| 团队队列 | 在该团队中开启接待且工作中的成员内按接待量分配；无人可分配时留在团队队列 |
| 公共队列 | 在企业内全部开启接待且工作中的成员中按接待量分配；无人可分配时留在公共队列 |
...
| 队列等待提醒 | 队列中的会话等待超时后提醒对应团队成员；公共队列提醒全部开启接待的成员 |
...
本方案让 AI 转人工和新进线的客户会话都有真人及时承接并回复。方案涉及接单与接待量、自动分配、积压补分配、未响应回收、队列等待提醒、工作时间和转人工话术。方案只写在现有转人工事务（`internal/actions/agentrun/customer_handoff.go`）之上的变化：接待资格按身份上的接待开关判定，`service_sessions.team_id` 已经是会话所属队列，收件箱「待分配」已按队列筛选。

##### 1. 已确定的决策

| 事项 | 决策 |
|---|---|
| 接单开关 | 复用成员现有的工作状态：只有「工作中」且开启接待的真人成员参与自动分配，「休息中」「下班了」不分配新会话。登录不再把工作状态重置为「工作中」 |
| 最大接待量 | 成员级字段，企业内一份；创建成员表单默认 10，成员详情可修改；只在开启接待的成员上显示与生效 |
| 当前接待量 | 本人负责的开放客服周期数 |
...
- 渠道路由的目标只有公共队列、团队、成员三种，AI 员工按成员配置。入站按首选、备用、公共队列依次解析（`chatstate.ResolveNewSessionRoute`），转人工只解析备用路由（`chatstate.ResolveHandoffRoute`）。路由到团队或公共队列后不会分配给任何人。
- 工作状态（`organization_identities.work_status`）只在前端用于控制新消息通知，路由与分配完全不读取；登录时被强制改为 `working`（`auth/login.go`）。
- 停用或关闭接待开关后退回原队列并写入 `service_session_returned` 已经实现；本方案只负责退回后的重新分配。
...
- 真人身份，账号有效，开启接待（沿用 `identity.ApplyCustomerHandlingConditions` 的有效接待身份判定）；
- 工作状态为「工作中」；
- 当前接待量小于最大接待量；
- 团队队列时是该团队成员，公共队列时是企业内任一开启接待的真人成员。

候选按当前接待量升序、`last_service_assigned_at` 升序（空值优先）、身份编号排序，取第一位。AI 员工不参与队列分配，其工作状态也不参与路由。未开启接待的成员既不参与分配，也无法领取或回复。
...
| 真人成员，开启接待且工作中 | 分配给该成员 | 分配给该成员 |
| 真人成员，开启接待但不在工作中 | 改用备用路由；备用路由不可用时进入公共队列 | 进入公共队列 |
| AI 员工 | 按现有资格判定 | 进入公共队列（现有） |

团队队列中无人可分配时，周期保持 `team_id = 团队`、负责人为空；公共队列时二者都为空。自动分配成功后 `team_id` 保留为该周期所属的队列，用于回收时退回原队列。领取、接管、转交给个人不改变 `team_id`；转交给团队或公共队列同时改变队列并清空负责人。
...
1. 在会话锁之外读出候选列表；
2. 依次对候选的 `organization_identities` 行取 `FOR NO KEY UPDATE`，在锁内复核工作状态、接待开关、账号状态并重新统计接待量，不满足时释放并尝试下一位；
...
| 真人停用、真人关闭接待开关 | 退回原队列已经实现，本方案在该事务内为退回的周期逐个投递分配任务 |
...
| `service_session_returned` | 未响应回收 | 原负责人、退回的队列，新增原因 `response_timeout`；事件与其时间线渲染已经实现 |
...
| 位置 | 变化 |
|---|---|
| 创建成员表单 | 开启接待时增加必填「最大接待量」，默认 10，正整数；未开启接待时不显示 |
| 成员详情 | 开启接待的成员增加「最大接待量」编辑行 |
| 工作状态菜单（桌面端导航、移动端「我的」） | 保持现有三项；「工作中」的帮助文案对开启接待的成员说明会被分配新的客户会话 |
| 系统设置 | 新增「客服」页签（`/settings/customer-service`），两个分组：「分配与提醒」维护三个时长；「工作时间」维护启用开关、时区、每周时段和日期覆盖 |
| 会话时间线 | 渲染 `service_session_assigned`，并为 `service_session_returned` 补上未响应回收的原因文案 |
...
| PR | 范围 |
|---|---|
| A 接待量与自动分配 | 3.1、3.2 的字段；登录不重置工作状态；4.1 至 4.5 全部分配入口与事件；退回队列后的重新分配；`service_attention` 的 `assigned` 通知及各端投递；成员表单与详情的最大接待量 |
...
- 收件箱「全部」范围口径与「待分配」状态限制：见「收件箱视图口径」。
...
- 需要生成或抽取原文的任务使用对话模型，例如小结正文、联系人资料、工单字段。
...
按六批交付：设备与派发、运行时编入、只读本机工具、写入与命令审批、群聊绑定、界面操作。

数据结构、契约、租约语义、工具与审批规则、安全边界、批次验收见 [桌面端本地 Agent 运行实施方案](desktop-local-agent.md)。
...
- 密钥加密存储与接口响应脱敏，范围包含托管部署的运营服务凭据与平台对象存储凭据。
...
| 对象存储 | 托管由平台提供存储并按企业前缀隔离，企业不自配；自托管保留企业配置 |

启动条件：官方身份服务、官网域名和首期商业规则确定后开始，见该方案的实现前定稿项。
