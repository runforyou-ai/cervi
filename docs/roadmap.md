...
1. 客服闭环：会话小结、运营报表与知识缺口、网站显式评价 → 访客与客户信息 → 第一批渠道与 AI 翻译 → 网站 Messenger 占位能力、AI 满意度与质检、联系人资料、常用语 → 第二批渠道、自定义 API 渠道、开放接口与权限。
...
- 咨询分类、客户情绪和是否解决由判断模型标注，小结正文由对话模型生成。
...
- 工作台一级导航为收件箱、AI 员工、通讯录，群聊与单聊直接列在一级栏；渠道、知识库与工具归入 AI 员工模块，二级栏固定为四个入口。

| 阶段 | 内容 | 前置 |
|---|---|---|
| A 工作台整理 | 交接卡片与 AI 过程折叠、渠道页只显示已接入、一级导航加入 AI 员工模块并合并渠道、知识库与工具、成员账号移入设置 | 与客服闭环并行 |
