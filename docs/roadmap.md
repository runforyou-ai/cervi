...
2. 工作台信息架构：阶段 A 的前端整理与客服闭环并行，阶段 B1 起在客服闭环交付后启动，见 [工作台信息架构与服务对象扩展方案](workbench-ia.md)。
...
| 移动端通讯录分类 | AI 员工、团队和外部联系人分类是占位 | 移动端需要按这些分类找人；分类与桌面端对齐为同事、团队、外部联系人、我的助理，AI 员工只以服务台形式列在同事中 |
...
- 不建设通用规则引擎；只提供确定性规则，即已上线的负责人未响应提醒与回收，以及 AI 负责时的超时跟进与超时关单。
...
- 转人工时由同一项摘要能力生成交接摘要（对方要什么、AI 已做什么、卡在哪），在会话详情顶部以交接卡片展示，领取后收起。
- 咨询分类沿用企业咨询分类目录，写入周期已有的咨询分类字段：转人工时已由 AI 选择的分类可被小结修正，未转人工的周期由小结补写。
- 咨询分类、客户情绪和是否解决由企业配置的判断模型标注，小结正文由对话模型生成。
- 概率低于阈值的判断结果不自动写入，留空或进入待复核。
...
| 结束方式 | 按客服周期的结束方式统计 AI 解决、客户失联、人工关闭的占比 |
| 解决情况 | 按「会话小结与分类」中小结的是否解决统计解决率，可按负责人类型、渠道和分类拆分；是否解决留空的周期单列 |
| AI 独立解决率 | 结束方式为 AI 解决（客户已确认）且该周期内未发生真人领取、接管、转交或真人对客回复的周期占比，作为对外口径 |
| 客户评价 | 网站访客对客服周期的评价（是否解决与评语）单独统计，与结束方式和小结对照展示，不覆盖二者 |
| 转人工原因 | 按 `outcome_reason` 分业务原因（知识不足、客户要求真人、需要人工判断、投诉）与系统原因（依据不足、预算耗尽、运行失败等）两组统计 |
| 知识缺口 | 原因为 `knowledge_gap` 与 `insufficient_evidence` 的转人工会话聚类为问题清单，标注出现次数与示例会话，可一键进入知识库补充问答 |
...
- 是否产生服务周期由入口（渠道或 AI 员工）声明的服务对象与发起人身份共同决定，与首接待是 AI 还是真人无关。渠道会话天然产生；成员对服务对象不含自己的 AI 员工是试聊，与助理的对话也不产生。
- 提出请求的一方看到聊天，处理的一方看到收件箱，同一条会话不复制数据。
- 转人工交出责任，AI 只交给团队或公共队列，去向依次取咨询分类团队、渠道失败去向或 AI 员工的「办不了交给谁」、公共队列；真人之间转交可点名到人。请示保持责任，属于暂缓事项。
- 不属于任何团队的成员不显示收件箱；以员工为服务对象的 AI 员工在通讯录中以服务台列出；涉及个人信息的 AI 员工可设为只在单聊受理。

| 阶段 | 内容 | 前置 |
|---|---|---|
| A 工作台整理 | 成员账号移入设置、建群名称可选 | 与客服闭环并行 |
| B1 服务会话底座 | 服务对象、服务会话泛化、转人工去向调整、可见性按角色 | 阶段 A；客服闭环交付 |
| B2 员工接入底座 | 员工目录同步、身份范围注入、群话题服务周期 | B1 |
| C 员工服务台 | 一级导航按职责显示、通讯录服务台、发起人服务状态、内部 IM 机器人渠道、IT 知识问答与转人工承接，其后 MCP 查询、身份系统执行与主管审批；验收时判断是否需要工单 | B2；审批依赖 P1.5 |
| D 伙伴服务台 | 伙伴组织与成员、身份绑定、按伙伴路由、业务系统查询类工具 | B2；需要开放查询接口的真实客户 |

与其他事项的关系：渠道按「渠道接入」的内部 IM 机器人批次交付；伙伴进入内部会话协作见「受管外部协作」；工具审批依赖「P1.5：服务端副作用、审批和恢复」；数据范围随「开放接口与权限」的角色权限一并收口。

## 4. AI 客服与客服 AI 辅助

运营报表、知识缺口、回答质量评测、经验沉淀与满意度见「客服运营与渠道」。
...
| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| AI 员工能力配置化 | AI 员工不分配企业角色，内置工作规则与可用工具由接待开关派生，在运行配置中只读展示 | 与外部业务系统读写授权矩阵一并建设 |
