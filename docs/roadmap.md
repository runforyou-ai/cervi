...
3. Agent：以 Eino 为参照先接入行业标准能力（长期记忆等）并推进助理与桌面端本地运行（P4），约束层逐步完善；与客服闭环并行推进，不占用同一批业务改动。
...
- 收件箱按成员能否接待服务会话显示，不按团队判断；涉及个人信息的 AI 员工可设为只在单聊受理。

| 阶段 | 内容 | 前置 |
|---|---|---|
| B 服务会话与 Cervi 单聊服务台 | IT 知识问答与转人工承接验收 | — |
...
1. 长期记忆，先在助理上贯通写入、召回、查看与删除，再扩到 AI 员工按客户隔离。
2. 本机 Agent：助理经 ACP 交给主人电脑上的 Agent 完成，见「P4：助理与桌面端本地运行」。

后台任务、工具检索与 AI 员工的 Skills 在出现真实场景时接入。
...
- 项目级技能见 [助理与桌面端本地运行实施方案](desktop-local-agent.md)「技能」。

### 后台任务

- 接入 `backgroundtask`：长时间命令与子 Agent 在后台运行，生命周期绑定所在 Run。设备长任务由客户端工作项承担，服务端唤醒仍由 `task_runs` 承担。
- 启动条件：助理出现命令超时，或单次运行内需要多个子任务并行的反馈。
- 面向客户的 AI 员工暂不接入任务清单、子 Agent 与后台任务：它们会增加回复延迟与费用，客服场景出现明确需要时按配置开启。

### 长期记忆

- 接入 `automemory`，记忆内容由 Cervi 持久化并可由用户查看与删除，检索复用本地知识库的转换与向量管线。
- 记忆按归属隔离：助理的记忆只属于其主人；AI 员工的记忆按服务对象归属：外部客户按客户身份（网站 Messenger 签名身份的企业用户编号），本企业员工与伙伴按其成员身份，一位服务对象的记忆不出现在其他服务对象的对话中。
...
设备注册、派发领取、运行时编入、模型代理、助理身份、群聊中的助理、本机运行的知识检索、附件和过程流、本机文件读写与命令执行、技能、企业与本地 MCP 和群内助理展示已上线，剩余本机 Agent。界面操作暂缓。
