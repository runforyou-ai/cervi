...
1. 客服闭环：首发市场为海外，首发渠道为网站与 Telegram。会话小结、AI 表现报表与知识回流 → AI 回复复核 → 离线回复通知、AI 翻译与多语言内置指令 → 网站 Messenger 占位能力、常用语、联系人资料、AI 满意度与质检、真人运营指标 → 海外第一批渠道 → 国内渠道、第二批渠道、自定义 API 渠道、开放接口与权限。
2. 工作台信息架构：阶段 B1 起在客服闭环交付后启动，见 [工作台信息架构与服务对象扩展方案](workbench-ia.md)。
3. 服务对象扩展：员工服务台（IT 先行），伙伴服务台在出现真实客户时启动，同上方案。
4. Agent：以 Eino 为参照先接入行业标准能力（Skills、上下文摘要、任务规划、子 Agent、长期记忆等）并推进助理与桌面端本地运行（P4），约束层逐步完善；与客服闭环并行推进，不占用同一批业务改动。
...
- 助理单聊会话头展示在线状态（离线、暂停、未绑定），与桌面端 `presenceLabel` 同一套文案。
- 助理单聊会话头提供工作区入口，复用 `ConversationAssistantWorkspace` 的非绑定设备分支：展示当前工作区并可清除。移动端不是助理绑定电脑，不提供选择或添加工作区，新建助理会话时也不选择工作区。
- 单聊与 AI 会话增加资料子页，内容与桌面端会话侧栏的内部会话资料一致：身份类型与 AI 运行状态。入口放在会话头，与客户会话的资料子页结构一致。
- 验收：桌面端为助理会话设置工作区后，移动端会话头显示该工作区，移动端清除后桌面端同步清除。
...
- 客服周期关闭时由 AI 生成小结，并标注咨询分类和是否解决；客服可修改，客服修改后的结果不再被 AI 覆盖。结束方式为客户失联时，是否解决留空；结束方式为 AI 解决（客户已确认）时，是否解决直接记为已解决。
- 转人工时由同一项摘要能力生成交接摘要（对方要什么、AI 已做什么、卡在哪），在会话详情顶部以交接卡片展示，领取后收起。
- 咨询分类沿用企业咨询分类目录，写入周期已有的咨询分类字段：转人工时已由 AI 选择的分类可被小结修正，未转人工的周期由小结补写。
- 咨询分类和是否解决由企业配置的判断模型标注，小结正文由对话模型生成；概率低于阈值的判断结果留空。
- 小结注入同一客户下次来访时 AI 与客服的上下文，作为「客户档案注入」的历史会话摘要来源。周期重开后再次关闭时重新生成。

### AI 表现报表与知识回流（P1）

| 事项 | 做法 |
|---|---|
| 解决情况 | 按小结的是否解决统计解决率，可按负责人类型、渠道和分类拆分；是否解决留空的周期单列。随「会话小结与分类」交付 |
| 知识缺口聚类 | 由判断模型把统计范围内已结束会话中、原因为 `knowledge_gap` 与 `insufficient_evidence` 的转人工的客户提问按相近问题聚类，标注出现次数与示例会话 |
| 问答草稿 | 转人工后真人已答复的问题，由 AI 按真人回复起草知识库问答条目，管理员确认后入库；知识缺口清单中的问题可直接查看对应草稿 |

AI 客服运行结果是报表与知识回流的事实来源。报表对企业成员全员可见，数据范围随权限统一建设。

### 满意度与质检

| 事项 | 优先级 | 做法 |
|---|---|---|
| AI 回复复核 | P1 | AI 负责并关闭的周期中，访客评价未解决或判断模型标记可能答错的，进入复核清单；客服复核后可直接生成问答草稿 |
| 其他渠道评价 | P2 | 渠道不支持交互时降级为文本回复 |
| AI 推断满意度 | P2 | 由判断模型推断满意度，覆盖全部客服周期；解决情况沿用小结的是否解决 |
| AI 质检 | P2 | 真人与 AI 客服的会话都检查，由判断模型标记答错、态度问题、应转人工未转等情况，并入复核清单 |
...
| 事项 | 优先级 | 做法 |
|---|---|---|
| 离线回复通知 | P1 | 非工作时间转人工且访客未留邮箱时，由 AI 在对话中询问邮箱；访客不在线时，客服回复通过邮件通知访客。需要部署级邮件发送配置，与邮件渠道相互独立 |
...
- 首发市场的客户语言各异，转人工后客服侧显示客户消息的译文，客服用自己的语言回复，发送时翻译为客户语言。
- 原文与译文同时保存，时间线可切换查看。
- 网站与 Telegram 首发即支持，后续渠道接入时同步适配。
...
业务系统接入、AI 表现报表、知识回流、回复复核与满意度见「客服运营与渠道」。
...
| 事项 | 当前行为 | 启动条件 |
|---|---|---|
| 客户档案注入 | 客服上下文只含当前周期的对客消息，不注入客户档案、渠道与历史会话摘要 | 与「会话小结与分类」同期注入历史会话摘要；客户档案随联系人资料落地 |
...
| 多语言内置指令（P1） | 角色基线与场景规则只有中文版本，回复语言跟随客户 | 首发市场为海外，与「AI 翻译」同期补齐英文版本，按企业或渠道语言选用 |
...
1. 助理的命令执行、文件改动与 Skills，见「P4：助理与桌面端本地运行」。
...
设备注册、派发领取、运行时编入、模型代理、助理身份、本机运行的知识检索、附件和过程流以及只读本机工具已上线，剩余按三批交付：命令执行、文件改动与 Skills，企业 MCP 与群聊中的助理，本机 Agent。界面操作暂缓。
