...
- 在输出方案和提交 PR 前，必须调用 Pi CLI 审核并处理反馈，每一次小的改动不需要审核，最终审核 pr 时主要是检查是否有更优雅的实现方式，以及针对本次改动，是否有啰嗦的注释、反向引用、无必要的防御性逻辑和历史兼容逻辑、注释与实现不一致的问题，以及是否要加上必要的直述型注释和 `INFO`、`WARN` 等日志。
