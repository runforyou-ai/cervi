...
- 代码注释、文档字符串和数据库 `COMMENT` 直接描述职责、字段含义、执行行为或必要约束，使用简洁的直述型表达。
- 注释不得包含讨论痕迹、需求确认、方案解释、方案取舍或历史实现对比，不使用“避免……”“不是……而是……”“不再……”等反向表述解释设计；必要约束直接写明适用条件、执行规则或不变量。

## 跨端约定

- `appservice.Service` 是统一业务入口：服务端 Web 走 `DirectBackend`，桌面端和移动端走 API Proxy。Gin 只做对外 HTTP API 适配。
- 各端统一使用 Bearer Token，不使用 Cookie。登录令牌保存在 `localStorage`，API Proxy 把应用服务调用转成携带 Token 的 HTTP 请求。唯一例外是网站匿名访客：公开 Messenger 使用渠道级长期 Cookie（`cervi_visitor_<channel_id>`）恢复匿名身份。
...
- 执行修改前尽量先说明任务范围并询问用户是否开始；用户已明确要求立即执行或已对该范围给予确认时，不重复询问。
...
- 通用操作、分页、表格操作列和加载状态优先复用 `common` namespace，业务 namespace 不重复定义。通过 `useTranslation` 显式声明所需 namespace；同一个 `t` 使用多个 namespace 时，跨 namespace 引用使用带 namespace 的翻译键。
...
- 文案、数字显示格式、间距、对齐等不改变布局结构或交互流程的简单修改，无需专门做可视化验证，也不为此启动服务、浏览器或桌面端；检查代码差异，按需核对格式化结果即可。用户明确要求界面验证时除外。
- 新增页面、调整布局结构或交互流程时，主动验证界面效果，优先使用浏览器；涉及桌面端时使用 Wails MCP。
- 移动端开发优先通过 `wails3 task dev:mcp` 启动的 `mobile-preview` 小窗口验证；仅涉及 iOS 或 Android 特有能力时使用对应模拟器或真机。
- MCP 缺少所需方法时，可通过 Computer Use 直接控制桌面窗口或 iOS、Android 模拟器，完成更详细的验证。
- 本地开发验证账号：`ai.shellphy@gmail.com`，密码：`12345678`。可直接用于登录验证，无需另行确认。
- 第二个本地开发验证账号：`jack@jack.com`，密码：`12345678`。用于双账号群聊、成员访问与群主转让验证，可直接登录，无需另行确认。
- 验证界面时，遇到需要用户手动完成的步骤（如验证码或授权确认），应说明当前步骤并暂停相关操作，等待用户完成后继续验证；不得仅因此放弃界面验证或将其标记为无法完成。
- 每次界面验证结束后，关闭本次启动的服务端、客户端、Vite、MCP 等进程及其子进程，并确认对应端口已释放；只清理本次启动的服务，不关闭其他工作区进程或共享的 PostgreSQL、NATS。用户明确要求保留运行时除外。
...
- 表格操作列放在最右侧，列宽收窄到操作内容所需宽度，表头与按钮的左边缘对齐；固定布局表格为操作列保留紧凑宽度，列内同样左对齐。
- 同一列表操作列中的按钮和菜单项因权限、身份或记录状态不可用时，保留显示并设置 `disabled`，不通过隐藏改变每行的操作位置；禁用操作不得响应点击或键盘触发。
...
- 当前以贯通 MVP 主流程和验证产品价值为优先，不为尚未出现的生产规模问题预先增加配额、限流、复杂重试、降级、穷举式参数限制或防御性分支；实现保持可扩展的清晰边界，待准备正式上线时单独开展安全、容量和异常边界专项并集中补齐。已有明确约定的认证、企业数据隔离、事务一致性和业务幂等仍须遵守。
- 不考虑历史数据和旧接口兼容。改模型、迁移和接口时直接实现目标结构，不写旧数据回填、缺失记录兜底或双版本逻辑，除非任务明确要求。
- MVP 阶段优先建立长期正确、语义清晰的领域模型和接口契约；不为减少当前改动而使用借用字段语义、查询过滤、兼容分支或其他局部兜底掩盖模型问题。发现基础契约不合理时，直接调整数据模型、业务边界和调用链路，并删除被替代的旧实现。
- 上线前统一评估并补充查询性能索引；MVP 阶段迁移只保留主键和用于业务约束、幂等及并发正确性的唯一索引，不提前添加普通性能索引。
- 密钥加密存储、接口响应脱敏等安全加固暂不阻塞开发和审查。
- 角色权限后续统一建设；当前只校验已登录，不按管理员或普通成员限制功能。

### 国际化

- 后端用户可见文案统一由 `internal/i18n` 管理。语义一致的文案复用同一个 `Key`，共享键按共同语义命名，不按调用入口重复定义；仅字面相同而语境不同的文案保留独立键。
