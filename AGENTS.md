...
- 管理页面采用左对齐的可用宽度布局并保持统一留白。
- 工作台一级导航为固定窄轨：图标在上、短名在下，不展开不折叠；左上头像打开用户菜单。二级栏显示模块标题，消息页除外。
- 不使用面包屑。模块和分类由窄轨与二级栏表达；离开列表的子页在标题行右上角放置文字「返回」，不另起一行。需要扩展的设置页和渠道编辑页使用与 URL 同步的页签，不展示空页签。
