...
- 对象存储开启时，文件由客户端通过服务端签发的预签名请求直传，服务端不转发文件内容，Endpoint 使用客户端可访问的公开地址；对象存储关闭时，文件写入企业服务器的本地最终目录。文件选择后立即上传为临时文件，保存业务数据时在事务中激活；未激活文件默认 24 小时过期，由服务端定时清理。文件读取按各自记录的原始存储位置处理，不受当前开关影响。
