...
标准知识库的文档支持上传原件、在线编写和网页导入三类来源，见 [知识来源扩展方案](knowledge-source-expansion-plan.md)。`knowledge_documents.source_kind` 区分来源，在线编写的正文与网页抓取的快照保存在 `knowledge_document_contents`，上传原件仍每次处理时转换。处理任务按来源决定前两个阶段：在线文档直接进入切分；网页在创建、重新抓取或尚无快照时由 `internal/integration/webfetch` 出网抓取后送 markitdown 转换并写入快照，重试与知识库重建读取已有快照，不重新抓取页面。文档名称、内容类型和字节数按来源取值，检索与分段查询左联结原件，来源名称取文档标题或原件文件名。
