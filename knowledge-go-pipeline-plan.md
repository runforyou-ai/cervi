...
知识库保存时，向量模型供应商、模型标识、向量维度或标准库的分段长度、分段重叠任一变化，即在同一事务中锁定全部文档和问答条目，删除本库全部分段、清除已发布批次，并按新配置为每个来源生成新处理标识后投递索引任务。每个来源发布新批次后恢复检索，尚无来源完成时该库检索返回未就绪；被替代的在途任务在发布事务中核验处理标识后放弃写入。召回数量、相关性阈值和重排模型只影响后续检索，不触发重建。
...
文档分段按转换后的 Markdown 结构切分：`textsplit.Split` 在收缩范围内依次优先选择块边界（标题行、空行、表格与代码块首尾）、换行、句末标点（中文 `。！？`，英文 `.!?` 后接空白）、分句标点（`；，`，英文 `;,` 后接空白）和空白，同等级取最接近均分目标的位置，距离相同时取更靠前的位置；前一正文行以文字结尾、本行以文字或句中标点开头的换行及其间空行视为 PDF 等来源的排版折行，按空白处理，没有句末标点的正文段落之间的空行同样按此处理。表格与代码块内部只在行间切分，单行超长时才使用行内边界。分段长度是目标长度：上限外留 20% 余量，到下一个标题的剩余正文在余量内时整体收入；余量内有块边界时，按到下一个标题的剩余正文估算段数和均分目标，选择最接近目标的块边界；当前块剩余正文超出余量时，按块内剩余正文估算均分目标，在长度上限内选择等级最高且最接近目标的边界；距下一个标题不超过长度 20% 的边界不参与选择。独立标题是硬边界：分段不跨越标题，下一个标题在余量内时直接切到标题处，紧随标题的下级标题与其合为一体；分段起点不是标题、且其后首个标题前的前导内容不超过长度 20% 时，该前导内容并入这个标题所在分段，只合并紧接着的一个标题；在标题处切开时不带重叠，其余位置的重叠起点对齐到重叠范围内等级最高且最靠前的边界，重叠长度不超过设定值；重叠范围内只有表格或代码块的行内边界时不带重叠。每个分段在 `knowledge_segments.context` 保存起点所属的标题路径和表头，正文保持原文连续区间；生成向量、词法词元和重排时使用「上下文 + 空行 + 正文」，分段查看、检索测试与 Agent 读取单独返回上下文。切分规则变化不触发自动重建，已发布文档在重试或知识库索引参数变更后按新规则生成分段。

标准知识库的文档支持上传原件、在线编写和网页导入三类来源，见 [知识来源扩展方案](knowledge-source-expansion-plan.md)。`knowledge_documents.source_kind` 区分来源，在线编写的正文与网页抓取的快照保存在 `knowledge_document_contents`，上传原件仍每次处理时转换。处理任务按来源决定前两个阶段：在线文档直接进入切分；网页在创建、重新抓取或尚无快照时由 `internal/integration/webfetch` 出网抓取，抓取结果按 BOM、页面内声明和响应头判定编码解码为 UTF-8，再由 readability 提取正文主体去掉导航、侧栏和页脚，之后送 markitdown 转换并写入快照；提取不到正文时送整页。重试与知识库重建读取已有快照，不重新抓取页面。文档名称、内容类型和字节数按来源取值，检索与分段查询左联结原件，来源名称取文档标题或原件文件名。
