...
知识库保存时，向量模型供应商、模型标识、向量维度或标准库的分段长度、分段重叠任一变化，即在同一事务中锁定全部文档和问答条目，删除本库全部分段、清除已发布批次，并按新配置为每个来源生成新处理标识后投递索引任务。每个来源发布新批次后恢复检索，尚无来源完成时该库检索返回未就绪；被替代的在途任务在发布事务中核验处理标识后放弃写入。召回数量和重排模型只影响后续检索，不触发重建。
