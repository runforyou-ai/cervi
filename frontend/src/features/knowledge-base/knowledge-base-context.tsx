...
type KnowledgeBaseContextValue = {
  ...
  documentListScrollPositions: Map<string, number>
...
export function KnowledgeBaseProvider({
  ...
  const documentListScrollPositions = useRef(new Map<string, number>()).current
  const qaListScrollPositions = useRef(new Map<string, number>()).current
  return (
    <KnowledgeBaseContext.Provider
      value={{ upsertKnowledgeBase, qaListScrollPositions, documentListScrollPositions }}
    >
      {children}
    </KnowledgeBaseContext.Provider>
  )
