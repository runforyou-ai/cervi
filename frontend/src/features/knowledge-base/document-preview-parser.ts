...
import { convertODT } from "./document-preview-odt.ts"
...
/** 根据原文件扩展名解析预览内容，MDX 只作为 Markdown 阅读。 */
export async function parseDocumentPreview(name: string, bytes: Uint8Array<ArrayBuffer>): Promise<DocumentPreview> {
  const extension = name.split(".").pop()?.toLowerCase()
  switch (extension) {
    case "pdf": return { kind: "pdf", content: bytes }
    case "md": case "markdown": case "mdx": return { kind: "markdown", content: decodeDocumentText(bytes) }
    case "txt": case "properties": case "vtt": return { kind: "text", content: decodeDocumentText(bytes) }
    case "html": case "htm": return { kind: "html", content: previewHTML(decodeDocumentText(bytes)) }
    case "docx": {
      const { default: mammoth } = await import("mammoth/mammoth.browser.js")
      const result = await mammoth.convertToHtml({ arrayBuffer: bytes.buffer })
      return { kind: "html", content: previewHTML(result.value) }
    }
    case "odt": return { kind: "html", content: previewHTML(await convertODT(bytes)) }
    case "xls": case "xlsx": case "csv": {
      const xlsx = await import("xlsx")
      // 旧版 XLS 的中文代码页随解析器按需加载。
      ...
      return { kind: "sheets", sheets: workbook.SheetNames.map((name) => ({ name, content: previewHTML(xlsx.utils.sheet_to_html(workbook.Sheets[name])) })) }
    }
