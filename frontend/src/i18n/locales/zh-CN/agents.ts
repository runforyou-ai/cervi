/** 简体中文·AI 员工文案。 */
const agents = {
  title: "AI 员工",
  description: "管理 AI 员工的身份、知识和运行配置",
  navigation: {
    label: "AI 员工模块",
    performance: "表现",
    agents: "AI 员工",
    channels: "渠道",
    knowledgeBases: "知识库",
    tools: "工具",
  },
  search: "搜索 AI 员工",
  sendMessage: "发消息",
  loadError: "AI 员工列表加载失败。",
  empty: "没有符合条件的 AI 员工",
  columns: {
    name: "名称",
    model: "对话模型",
    addedAt: "添加时间",
  },
  addedAt: "{{time}} 添加",
  configure: "配置",
  nameSeparator: "、",
  editTitle: "配置 AI 员工",
  createDescription: "创建 AI 员工并配置身份与知识",
  editDescription: "调整 AI 员工的身份、知识和运行配置",
  edit: "配置 AI 员工 · {{name}}",
  basic: "基本资料",
  create: "添加 AI 员工",
  form: {
    name: "AI 员工名称",
    serviceAudiences: "服务对象",
    serviceAudiencesHelp: "服务客户时接待渠道会话，服务员工时受理同事在 Cervi 中提出的请求。",
    audiences: {
      customer: "客户",
      employee: "员工",
    },
    handoffTeam: "办不了交给谁",
    handoffTeamHelp: "只对 Cervi 内的单聊和群话题生效，渠道会话按渠道设置转交。",
    publicQueue: "公共队列",
    responsible: "负责人",
    responsibleNone: "不指定",
    responsibleOption: "{{name}}（{{email}}）",
    responsibleInactiveOption: "{{name}}（{{email}}，已停用）",
    responsibleHelp: "负责维护知识和答复质量，服务台在通讯录中显示负责人。",
    teams: "所属团队",
    noTeams: "还没有可选择的团队。",
    created: "AI 员工已添加",
    loadError: "AI 员工配置加载失败，请重试。",
    networkError: "无法连接服务器，请稍后重试。",
  },
  validation: {
    nameRequired: "请输入 AI 员工名称。",
    nameInvalid: "AI 员工名称只能包含文字、数字、空格和 · - _ . 符号。",
    modelRequired: "请选择对话模型。",
    instructionTooLong: "企业指令不能超过 20000 个字符。",
  },
  mcp: {
    services: "MCP 服务",
    title: "配置 MCP 服务",
    unconfigured: "未配置",
    selected: "已选择 {{count}} 个服务",
    selectedOne: "已选择{{names}}",
    selectedNames: "已选择{{names}}等 {{count}} 个服务",
    empty: "暂无 MCP 服务",
    loadError: "MCP 服务列表加载失败，请重试。",
    tools: "{{count}} 个工具",
  },
  execution: {
    knowledgeBases: "知识库",
    knowledgeEmpty: "暂无知识库",
    knowledgeTitle: "配置知识库",
    knowledgeUnconfigured: "未配置",
    knowledgeSelected: "已选择 {{count}} 个知识库",
    knowledgeSelectedOne: "已选择{{names}}",
    knowledgeSelectedNames: "已选择{{names}}等 {{count}} 个知识库",
    knowledgeLoadError: "知识库列表加载失败，请重试。",

    title: "运行配置",
    model: "对话模型",
    instruction: "企业指令",
    instructionHelp:
      "内置工作规则已包含通用要求，这里只写企业特有的业务背景、语气称呼、特殊政策和禁止事项。",
    behavior: "内置工作规则",
    saveError: "保存运行配置失败，请重试。",
    modelSelect: "选择对话模型",
    modelLoading: "正在加载模型…",
    modelLoadError: "读取对话模型失败。",
    noModels: "还没有可用的对话模型。",
    configureModels: "前往配置模型服务",
    configureModelsOnDesktop: "请在电脑上的 Cervi 中配置模型服务。",
  },
  status: {
    deactivate: "禁用",
    reactivate: "恢复正常",
    deactivateTitle: "禁用“{{name}}”？",
    reactivateTitle: "将“{{name}}”恢复正常？",
    deactivateDescription:
      "禁用后不能向该 AI 员工发送新消息，已提交的任务会继续执行；相关渠道分配将重置为公共队列，团队关系和历史会话保留。",
    reactivateDescription:
      "恢复正常后仍保持“下班”，团队关系不变，渠道分配需要重新配置；需要时请再切换为“工作中”。",
    saving: "正在处理…",
    deactivated: "AI 员工已禁用",
    reactivated: "AI 员工已恢复正常",
    error: "修改 AI 员工账号状态失败，请重试。",
  },
  performance: {
    title: "表现",
    description: "查看 AI 客服解决了多少、为什么转人工、缺哪些资料",
    loadError: "AI 表现报表加载失败。",
    tabs: {
      overview: "概览",
      knowledgeGaps: "待补知识",
      channels: "渠道",
      categories: "咨询分类",
    },
    gapQuestion: "客户提问",
    gapTime: "时间",
    gapStatus: "状态",
    gapStatuses: {
      pending: "待处理",
      accepted: "已加入知识库",
      dismissed: "已忽略",
    },
    gapSources: {
      knowledge_gap: "转人工：知识不足",
      insufficient_evidence: "转人工：缺少依据",
      rated_unresolved: "访客评价未解决",
      possibly_wrong: "可能答错",
    },
    gapTimes: {
      knowledge_gap: "{{time}} 转人工",
      insufficient_evidence: "{{time}} 转人工",
      rated_unresolved: "{{time}} 评价",
      possibly_wrong: "{{time}} 结束",
    },
    hasDraft: "已生成草稿",
    viewConversation: "查看对话",
    dismissGap: "忽略",
    period: "结束时间",
    periodDays: "近 {{count}} 天",
    channel: "渠道",
    allChannels: "全部渠道",
    empty: "—",
    aiResolvedRate: "AI 独立解决率",
    aiResolvedDetail: "{{resolved}} / {{closed}} 个已结束会话",
    ratingRate: "客户评价已解决",
    ratingDetail: "{{formatted}} 条评价",
    handoffRate: "转人工率",
    handoffRateDetail: "{{formatted}} 个会话转人工",
    gaps: "待补知识",
    gapsDetail: "等待整理成问答的问题",
    resolution: "解决情况",
    resolutionGroup: "{{name}} · {{formatted}} 个会话",
    aiOnly: "AI 独立处理",
    humanInvolved: "有真人参与",
    resolved: "已解决",
    unresolved: "未解决",
    undetermined: "未判定",
    closeReasons: "结束方式",
    closeAIResolved: "AI 解决",
    closeCustomerUnresponsive: "客户失联",
    closeManual: "人工关闭",
    handoffReasons: "转人工原因",
    businessReasons: "业务原因",
    systemReasons: "系统原因",
    noHandoffs: "这段时间没有转人工",
    uncategorized: "未分类",
    columns: {
      name: "名称",
      closed: "已结束",
      resolved: "已解决",
      aiResolved: "AI 独立解决",
    },
    noSessions: "这段时间没有结束的会话",
    noGaps: {
      pending: "这段时间没有待处理的待补知识",
      accepted: "这段时间没有加入知识库的条目",
      dismissed: "这段时间没有忽略的条目",
    },
    noQuestion: "（非文字消息）",
    gapSheet: {
      title: "补充知识",
      loadError: "待补知识加载失败。",
      conversation: "对话",
      openConversation: "查看完整对话",
      customer: "客户",
      knowledgeBase: "知识库",
      noKnowledgeBase: "还没有问答知识库，先在知识库中创建一个。",
      similar: "知识库已有相似问答：「{{question}}」",
      merge: "并入这条",
      merging: "将更新已有问答「{{question}}」",
      createNew: "改为新建",
      mergeError: "读取已有问答失败。",
      drafting: "AI 正在整理问答…",
      draftFailed: "AI 没能整理出问答，请直接填写。",
      draftUnavailable: "未设置会话小结模型，AI 不会整理问答，请直接填写。",
      draftArrived: "AI 已整理好问答。",
      useDraft: "替换为 AI 草稿",
      accept: "加入知识库",
      accepting: "正在加入…",
      acceptSuccess: "已加入知识库",
      acceptError: "加入知识库失败。",
      dismissError: "忽略失败。",
      accepted: "已加入知识库",
      viewEntry: "查看问答",
    },
  },
}

export default agents
