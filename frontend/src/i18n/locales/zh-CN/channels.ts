/** 简体中文·渠道文案。 */
const channels = {
  types: {
    website: "网站",
    telegram: "Telegram",
    wechatOfficialAccount: "微信公众号",
  },
  plannedTypes: {
    wechatCustomerService: "微信客服（企业微信）",
    whatsapp: "WhatsApp",
    facebookMessenger: "Facebook Messenger",
    instagram: "Instagram 私信",
    email: "邮件",
    line: "LINE",
    customApi: "自定义 API",
    douyin: "抖音企业号",
    xiaohongshu: "小红书专业号",
    kuaishou: "快手私信",
    sms: "短信",
    discord: "Discord",
    viber: "Viber",
    kakaotalk: "KakaoTalk",
    zalo: "Zalo",
    appSdk: "移动 App SDK",
    taobao: "淘宝／天猫",
    jd: "京东",
    pinduoduo: "拼多多",
    phone: "电话与呼叫中心",
    x: "X 私信",
    appleMessages: "Apple Messages for Business",
  },
  filters: {
    status: "渠道状态",
  },
  statuses: {
    enabled: "已启用",
    disabled: "已停用",
  },
  tabs: {
    basic: "基本信息",
    reception: "接待设置",
    chatInterface: "聊天界面",
    helpCenter: "帮助中心",
    usage: "接入方式",
    connection: "连接",
  },
  list: {
    title: "渠道",
    description: "管理客户发起会话的接入渠道",
    create: "添加渠道",
    activate: "启用",
    deactivate: "停用",
    statusUpdateError: "切换渠道状态失败，请重试。",
    loadError: "渠道加载失败。",
    emptyTitle: "还没有渠道",
    emptyFiltered: "没有符合筛选条件的渠道",
    columns: {
      name: "名称",
      addedAt: "添加时间",
    },
    addedAt: "{{time}} 添加",
  },
  deactivation: {
    title: "停用“{{name}}”？",
    description: "停用后仍可重新启用。",
  },
  activation: {
    title: "启用“{{name}}”？",
    description: "渠道状态将变为已启用。",
  },
  create: {
    title: "添加{{type}}渠道",
    description: "填写渠道信息并完成接入",
  },
  edit: {
    title: "{{type}}渠道设置",
    description: "维护渠道的基础信息、接待方式和接入设置",
    fallbackTitle: "渠道设置",
    namedTitle: "{{type}} · {{name}}",
  },
  form: {
    type: "渠道类别",
    name: "渠道名称",
    description: "说明",
    defaultLocale: "默认接待语言",
    loadError: "渠道详情加载失败。",
    networkError: "无法连接服务器，请稍后重试。",
  },
  routing: {
    newConversation: "新会话进入",
    fallback: "无法处理时",
    select: "请选择",
    person: "企业成员",
    agent: "AI 员工",
    loadError: "团队和成员加载失败，请重试。",
    targetLabels: {
      newConversation: {
        team: "接待团队",
        member: "接待成员",
      },
      fallback: {
        team: "转交团队",
      },
    },
    newConversationTypes: {
      public_queue: "公共队列",
      team: "指定团队",
      member: "指定成员",
    },
    fallbackTypes: {
      public_queue: "返回公共队列",
      team: "转交指定团队",
    },
  },
  validation: {
    nameRequired: "请输入渠道名称。",
    nameTooLong: "渠道名称不能超过 100 个字符。",
    descriptionTooLong: "渠道说明不能超过 2000 个字符。",
    teamRequired: "请选择团队。",
    memberRequired: "请选择成员。",
    fallbackDifferent: "无法处理时不能转回同一个团队。",
  },
  telegramConnection: {
    form: {
      botToken: "Bot Token",
      test: "测试连接",
      testing: "正在测试…",
    },
    tested: "连接测试成功。",
    saveError: "无法保存 Telegram 连接，请稍后重试。",
    testError: "连接测试失败，请稍后重试。",
    reuseConfirmation: {
      title: "确认复用 Telegram Bot？",
      description:
        "这个 Bot 已被其他渠道使用。继续保存后，Telegram 会把 Webhook 切换到当前渠道，旧渠道将无法继续接收通知。",
    },
    info: {
      title: "连接信息",
      botDisplayName: "机器人名称",
      botUsername: "机器人用户名",
      botId: "Bot ID",
      webhookUrl: "Webhook 地址",
      webhookSecret: "Secret Token",
      webhookStatus: "Webhook 状态",
    },
    status: {
      waiting: "等待连接",
      normal: "连接正常",
    },
    validation: {
      tokenRequired: "请输入 Bot Token。",
      tokenTooLong: "Bot Token 不能超过 512 个字符。",
    },
  },
  usage: {
    embed: "网站嵌入",
    link: "聊天链接",
    snippet: "安装代码",
    snippetHelp: "把这段代码放到网站页面中，右下角会出现聊天入口。",
    allowedHosts: "允许使用的网站",
    allowedHostsHelp: "每行填写一个域名。留空或填写 * 表示所有网站都可以使用。",
    chatUrl: "聊天链接",
    chatUrlHelp: "访客打开链接后会直接进入聊天页。",
    qrCode: "二维码",
    qrCodeHelp: "访客扫码后会打开这个聊天链接。",
    qrCodeAlt: "聊天链接二维码",
    qrCodeLoading: "正在生成…",
    qrCodeFailed: "二维码生成失败，请重试。",
    copy: "复制",
    copied: "已复制",
    copyFailed: "复制失败，请手动复制。",
    originError: "无法生成对外入口，请稍后重试。",
    validation: {
      allowedHostsTooMany: "允许使用的网站不能超过 50 个。",
      allowedHostInvalid: "请每行填写一个有效域名、HTTP(S) 地址或 *。",
    },
    instructions: {
      open: "查看使用说明",
      embedTitle: "如何添加到网站",
      embedDescription: "把安装代码添加到网站中，访客就能通过聊天入口发起咨询。",
      addCode: "添加安装代码",
      addCodeHelp: "复制下面的代码，粘贴到需要显示聊天入口的页面中，建议放在 </body> 前。",
      customButton: "使用网站自己的按钮",
      customButtonHelp: "页面上的按钮也可以通过下面的属性打开这个聊天窗口。",
      contactButton: "联系客服",
      verifyEmbed: "发布并验证",
      verifyEmbedHelp: "发布网站后打开对应页面，确认右下角聊天入口和页面按钮可以正常打开。",
      linkTitle: "如何使用聊天链接",
      linkDescription: "可以直接把聊天链接发给访客，也可以放在网站按钮或二维码中。",
      shareLink: "分享链接",
      shareLinkHelp: "复制下面的链接，通过消息发送给访客，或作为网站按钮的跳转地址。",
      useQrCode: "使用二维码",
      useQrCodeHelp: "展示当前页面中的二维码，访客扫码后会直接进入聊天页。",
    },
  },
  chatInterface: {
    form: {
      title: "聊天标题",
      greetingMessage: "欢迎语",
      themeColor: "主题色",
      colorPicker: "选择主题色",
      homeLinks: "首页链接",
      homeLinksDescription: "显示在 Messenger 首页，访客点击后在新标签页打开",
      homeLinkTitle: "标题",
      homeLinkURL: "链接地址",
      addHomeLink: "添加链接",
      moveHomeLinkUp: "上移第 {{number}} 个链接",
      moveHomeLinkDown: "下移第 {{number}} 个链接",
      removeHomeLink: "删除第 {{number}} 个链接",
    },
    validation: {
      titleRequired: "请输入聊天标题。",
      titleTooLong: "聊天标题不能超过 100 个字符。",
      greetingTooLong: "欢迎语不能超过 500 个字符。",
      themeColorInvalid: "请输入有效的六位十六进制颜色。",
      homeLinkTitleRequired: "请输入链接标题。",
      homeLinkTitleTooLong: "链接标题不能超过 100 个字符。",
      homeLinkURLInvalid: "请输入以 http:// 或 https:// 开头的有效链接地址。",
    },
    preview: {
      title: "实时预览",
      frameTitle: "访客 Messenger 预览",
      loading: "正在加载访客 Messenger…",
      loadFailed: "无法加载访客 Messenger 预览。",
    },
  },
  helpCenter: {
    knowledgeBases: "发布的知识库",
    knowledgeBasesHelp:
      "选中的知识库对该渠道的所有访客公开，只发布在线编写的文档和问答。未选择时不显示帮助中心。",
    pickerTitle: "选择发布的知识库",
    unconfigured: "未发布",
    selected: "已选择 {{count}} 个知识库",
    selectedOne: "已选择{{names}}",
    selectedNames: "已选择{{names}}等 {{count}} 个知识库",
    empty: "暂无知识库",
    loadError: "知识库列表加载失败，请重试。",
  },
}

export default channels
