...
test("共享正文支持中文、GFM 表格、任务列表和代码原文复制", async (t) => {
  ...
  api.render(container, { bodyFormat: "markdown", body: '# 标题\n\n**重点**\n第二行\n\n- [x] 完成\n- [ ] 待办\n\n| 名称 | 结果 |\n| --- | --- |\n| 测试 | 成功 |\n\n```js\nconst a = "<安全>";\n```' })
  await rendered(() => {
...
test("流式输入修复未闭合语法并保留已完成块，结束后与历史正文一致", async (t) => {
  ...
  api.render(container, { bodyFormat: "markdown", body: prefix + "**正在" }, true)
  ...
  for (let length = prefix.length + 5; length <= body.length; length += 7) {
    api.render(container, { bodyFormat: "markdown", body: body.slice(0, length) }, true)
    await new Promise((resolve) => setTimeout(resolve, 5))
    assert.equal(container.querySelector("p"), firstParagraph)
  }
  api.render(container, { bodyFormat: "markdown", body }, false)
  await rendered(() => {
  ...
  api.render(history, { bodyFormat: "markdown", body })
