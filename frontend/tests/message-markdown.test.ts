...
test("网站引用与 Markdown 正文共存，定位历史后保持阅读位置", async (t) => {
  ...
  window.fetch = async (path: string) => ({ ok: true, json: async () => {
    if (path.endsWith("/messenger")) return { visitorToken: "test", reception: { handlerType: null, handlerName: "", handlerAvatarUrl: "", online: false, reply: "soon", nextOpeningAt: null }, receptionRefreshAt: null, conversations: [{ id: "conversation", title: "测试", preview: "**人工正文**", previewSenderIdentityType: "user", lastMessageSeq: replies[1].messageSeq, lastMessageAt: replies[1].originatedAt }] }
