// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as domain$0 from "../domain/models.js";

/**
 * ChannelSummary 定义渠道选择项。
 */
export interface ChannelSummary {
    "id": string;
    "type": ChannelType;
    "name": string;
}

/**
 * ChannelType 表示渠道类型。
 */
export type ChannelType = domain$0.ChannelType;

/**
 * Contact 定义联系人完整详情。
 */
export interface Contact {
    "contact": ContactRecord;
    "sourceChannel": ContactSourceChannel;
    "methods": ContactMethod[] | null;
    "channelIdentities": ContactChannelIdentity[] | null;
}

/**
 * ContactChannelIdentity 定义联系人渠道身份。
 */
export interface ContactChannelIdentity {
    "channelId": string;
    "channelName": string;
    "externalId": string;
    "displayName": string | null;
}

/**
 * ContactInput 定义联系人可编辑字段。
 */
export interface ContactInput {
    "displayName": string;
    "channelId": string;
    "stage": ContactStage;
    "notes": string;
    "methods": ContactMethodInput[] | null;
}

/**
 * ContactList 定义联系人分页结果。
 */
export interface ContactList {
    "contacts": ContactSummary[] | null;
    "page": PageInfo;
}

/**
 * ContactListInput 定义联系人列表查询条件。
 */
export interface ContactListInput {
    "query": string;
    "stage": ContactStage;
    "channelId": string;
    "methodType": ContactMethodType;
    "sort": ContactSort;
    "page": number;
    "pageSize": number;
    "deleted": boolean;
}

/**
 * ContactMethod 定义联系人联系方式。
 */
export interface ContactMethod {
    "type": ContactMethodType;
    "value": string;
    "label": string | null;
    "isPrimary": boolean;
}

/**
 * ContactMethodInput 定义联系人联系方式输入。
 */
export interface ContactMethodInput {
    "type": ContactMethodType;
    "value": string;
    "label": string;
    "isPrimary": boolean;
}

/**
 * ContactMethodType 表示联系人联系方式类型。
 */
export type ContactMethodType = domain$0.ContactMethodType;

/**
 * ContactRecord 定义联系人详情字段。
 */
export interface ContactRecord {
    "id": string;
    "sourceChannelId": string;
    "displayName": string | null;
    "stage": ContactStage;
    "notes": string | null;
    "createdAt": string;
}

/**
 * ContactSort 表示联系人列表排序方式。
 */
export type ContactSort = domain$0.ContactSort;

/**
 * ContactSourceChannel 定义联系人来源渠道。
 */
export interface ContactSourceChannel {
    "id": string;
    "type": ChannelType;
    "name": string;
}

/**
 * ContactStage 表示联系人阶段。
 */
export type ContactStage = domain$0.ContactStage;

/**
 * ContactSummary 定义联系人列表项。
 */
export interface ContactSummary {
    "id": string;
    "displayName": string | null;
    "stage": ContactStage;
    "primaryEmail": string | null;
    "primaryPhone": string | null;
    "sourceChannelName": string;
    "createdAt": string;
    "deletedAt": string | null;
}

/**
 * Conversation 定义收件箱中的会话。
 */
export interface Conversation {
    "id": string;
    "name": string;
    "initials": string;
    "channel": string;
    "preview": string;
    "time": string;
    "status": string;
    "unread"?: number;
    "online"?: boolean;
    "messages": Message[] | null;
}

/**
 * DirectoryUser 定义企业成员目录字段。
 */
export interface DirectoryUser {
    "id": string;
    "email": string;
    "displayName": string;
    "role": UserRole;
    "status": UserStatus;
    "createdAt": string;
}

/**
 * Inbox 定义统一收件箱结果。
 */
export interface Inbox {
    "organization": Organization;
    "user": User;
    "conversations": Conversation[] | null;
}

/**
 * InstallWorkspaceInput 定义企业初始化输入。
 */
export interface InstallWorkspaceInput {
    "organizationName": string;
    "displayName": string;
    "email": string;
    "password": string;
}

/**
 * Locale 表示应用支持的本地化语言。
 */
export type Locale = domain$0.Locale;

/**
 * LoginInput 定义登录输入。
 */
export interface LoginInput {
    "email": string;
    "password": string;
}

/**
 * Message 定义收件箱会话中的消息。
 */
export interface Message {
    "id": string;
    "author": MessageAuthor;
    "text": string;
    "time": string;
}

/**
 * MessageAuthor 表示消息发送方。
 */
export type MessageAuthor = domain$0.MessageAuthor;

/**
 * Organization 定义当前企业信息。
 */
export interface Organization {
    "id": string;
    "name": string;
}

/**
 * PageInfo 定义分页信息。
 */
export interface PageInfo {
    "number": number;
    "size": number;
    "total": number;
}

/**
 * Principal 定义当前用户及其所属企业。
 */
export interface Principal {
    "organization": Organization;
    "user": User;
}

/**
 * RequestMeta 携带一次应用服务调用的认证和本地化信息。
 */
export interface RequestMeta {
    "token": string;
    "locale": Locale;
}

/**
 * S3Setting 定义 S3 兼容对象存储配置。
 */
export interface S3Setting {
    "enabled": boolean;
    "provider": StorageProvider;
    "endpoint": string;
    "region": string;
    "bucket": string;
    "accessKeyId": string;
    "secretAccessKey": string;
    "forcePathStyle": boolean;
}

/**
 * Session 包含登录身份和访问令牌。
 */
export interface Session {
    "principal": Principal;
    "token": string;
    "expiresAt": string;
}

/**
 * StorageProvider 表示 S3 兼容对象存储提供商。
 */
export type StorageProvider = domain$0.StorageProvider;

/**
 * User 定义当前企业成员信息。
 */
export interface User {
    "id": string;
    "organizationId": string;
    "email": string;
    "displayName": string;
    "role": UserRole;
    "status": UserStatus;
}

/**
 * UserList 定义企业成员分页结果。
 */
export interface UserList {
    "users": DirectoryUser[] | null;
    "page": PageInfo;
}

/**
 * UserListInput 定义企业成员列表查询条件。
 */
export interface UserListInput {
    "query": string;
    "status": UserStatus;
    "role": UserRole;
    "page": number;
    "pageSize": number;
}

/**
 * UserRole 表示企业成员角色。
 */
export type UserRole = domain$0.UserRole;

/**
 * UserStatus 表示企业成员状态。
 */
export type UserStatus = domain$0.UserStatus;

/**
 * WebsiteChannel 定义网站渠道详情。
 */
export interface WebsiteChannel {
    "id": string;
    "organizationId": string;
    "createdByUserId": string;
    "type": ChannelType;
    "name": string;
    "description": string | null;
    "defaultLocale": Locale;
    "createdAt": string;
    "updatedAt": string;
    "deletedAt": string | null;
    "chatInterface": WebsiteChannelChatInterface;
}

/**
 * WebsiteChannelChatInterface 定义网站渠道访客界面设置。
 */
export interface WebsiteChannelChatInterface {
    "title": string;
    "subtitle": string | null;
    "greetingMessage": string | null;
    "themeColor": string;
}

/**
 * WebsiteChannelChatInterfaceInput 定义网站渠道访客界面输入。
 */
export interface WebsiteChannelChatInterfaceInput {
    "title": string;
    "subtitle": string;
    "greetingMessage": string;
    "themeColor": string;
}

/**
 * WebsiteChannelInput 定义网站渠道可编辑字段。
 */
export interface WebsiteChannelInput {
    "name": string;
    "description": string;
    "defaultLocale": Locale;
}

/**
 * WebsiteChannelSummary 定义网站渠道列表项。
 */
export interface WebsiteChannelSummary {
    "id": string;
    "organizationId": string;
    "createdByUserId": string;
    "type": ChannelType;
    "name": string;
    "description": string | null;
    "defaultLocale": Locale;
    "createdAt": string;
    "updatedAt": string;
    "deletedAt": string | null;
}
