...
- 本地知识库直接接入 Haystack + Hayhooks，模型配置统一由 Cervi 后台管理，实施顺序与评测结论见 [本地知识库接入方案](knowledge-base-plan.md)。长期记忆也统一使用 Haystack + Hayhooks，交互尚未确定；该方案第 9 节记录职责与后续验收，不构成本轮实现范围。
...
| Automemory | 出现明确的跨 Run 长期记忆产品 | 先确定来源、使用范围与遗忘交互；统一使用 Haystack + Hayhooks 管线与后台模型配置；Cervi 管理事实、来源、纠正与遗忘，不能只留在 Eino Store |
...
- 完成前按 `ServiceSession -> State -> Run` 重新校验负责人、渠道、Agent 资格、Run Revision 和实际消费边界，接管、关闭或换负责人后的迟到结果不得写入。
