...
- Web 与桌面端 AI 聊天支持发送附件，包括草稿首发；成员在该会话已无上传中的附件时，以最新一条晚于已有输入来源的已完成附件追加一次输入，上传完成前已发送的文字所触发的运行不包含这些附件，附件随下一次输入进入上下文。AI 聊天、群聊和客服运行的上下文包含已上传完成的附件消息，附件以名称、类型、大小和稳定公开地址写入正文。模型声明支持的图片、音频或视频格式在按窗口推导的数量预算和字节上限内以 base64 通过多模态字段随消息直传，其余附件只提供正文中的链接；运行期不提供读取附件的工具。
