...
AI 客服的行为由代码约束：回答前先取得资料，没有资料只能追问或转人工，解决不了必须有出口。门禁保证的是「没有查证不能直接对客作答」，不保证回答忠实于资料；回答是否超出资料范围、追问文案是否夹带结论，由后续反馈回流与评测覆盖。后续事项见 [AI 客服后续事项](ai-customer-service-followup-plan.md)。
