...
| `copilot` | 客户会话 Copilot 线程中的成员消息自动触发 | 按客服 AI 辅助方案交付 |
...
客户会话中的 AI 协作由 Copilot 线程承担：线程是独立的 `copilot` 类型会话，按所属客户会话授权，执行范围为线程本身，只读引用客户会话作为背景，结果写回线程，不进入客户会话时间线，也不创建渠道投递；详见 [客服 AI 辅助方案](customer-ai-assist-plan.md) 第 4 节。
