...
- Web 与桌面端 AI 聊天支持发送附件，包括草稿首发；成员在该会话已无上传中的附件时，以最新一条晚于已有输入来源的已完成附件追加一次输入，上传完成前已发送的文字所触发的运行不包含这些附件，附件随下一次输入进入上下文；客户端在批次上传完成前异常退出时，未完成附件由过期文件扫描标记为失败，已完成附件同样随成员下一次输入进入上下文。AI 聊天、群聊和客服运行的上下文包含已上传完成的附件消息，附件以名称、类型、大小和稳定公开地址写入正文。模型声明支持的图片、音频或视频格式在按窗口推导的数量预算和字节上限内以 base64 通过多模态字段随消息直传，其余附件只提供正文中的链接；携带直传附件的模型调用失败时，去掉多模态内容、仅保留正文链接重新执行一次。运行期不提供读取附件的工具。
...
失败消息沿用消息时间线的发送者、头像、排序、分页与成员阅读状态。正文留空，客户端按 `agent_error` 类型显示本地化红色「出错了」；技术错误保存在 Run 中。模型上下文与引用接受文本消息和已上传完成的附件消息，网站访客历史和摘要仅展示正常聊天内容。会话列表通过 `lastMessageType` 生成特殊消息摘要。
