...
本工作区已支持人工与 AI 客服的私聊文本收发：入站复用客户会话，外发使用 `sendMessage` 和持久投递记录。Web、桌面收件箱统一使用时钟、对勾和提醒图标表示发送中、已发送和需要关注。本地提交、排队、平台调用和自动重试都归为发送中；失败、渠道停用和结果未确认通过提示说明具体原因，并提供人工重试、标记已发送和标记失败。
...
- Telegram 接待路由与人工转交支持 AI 客服，客户消息复用 customer_auto 与 Agent Revision 绑定的知识库；仅新消息调度，正常回答原子创建投递，模型失败只在内部时间线展示。
