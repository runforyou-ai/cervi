...
## 当前实现：Telegram 私聊文本双向收发与 AI 客服

本工作区已支持人工与 AI 客服的私聊文本收发：入站复用客户会话，外发使用 `sendMessage` 和持久投递记录。Web、桌面收件箱使用时钟和提醒图标表示发送中和需要关注，发送成功后不展示状态图标。本地提交、排队、平台调用和自动重试都归为发送中；失败、渠道停用和结果未确认通过提示说明具体原因，并提供人工重试、标记已发送和标记失败。

- 本地消息、投递及快速唤醒任务在同一事务中写入；数据库周期扫描负责恢复丢失唤醒。
- 按渠道身份分配严格递增的发送顺序；人工重试进入队尾。已发送状态表示 Telegram 已接受消息，或客服已人工确认，不表示客户已读。
- 429 根据平台等待时间暂停该渠道；超时、模糊的服务端错误和过期发送租约进入结果未知，等待后转人工处理，不自动重发。
- 停用渠道暂停待发送消息，启用后继续；更换 Bot 终止尚未发送的旧 Bot 消息。发送与配置变更共用渠道锁。入站幂等键和平台消息唯一约束包含 Bot 编号。
- Telegram 接待路由与人工转交支持 AI 客服，客户消息复用 customer_auto 与 Agent Revision 执行配置；本地知识库绑定保存在 Revision 中，检索尚未接通；仅新消息调度，正常回答原子创建投递，模型失败只在内部时间线展示。
- 接管、转交和关闭抑制未提交的 AI 回答；已提交投递继续发送。更换 Bot 同事务取消旧 AI 运行并推进输入水位，旧结果不会由新 Bot 发出。
- Telegram 仍不开放引用、媒体、群聊、频道或个人账号接入。Website 消息不创建外部投递。

验收使用 `wails3 task test:server`、`test:frontend`、`test:desktop`，以及前端、服务端和当前平台构建。真实 Bot 验收需要可访问的 HTTPS 回调地址、用户配置的测试 Bot 和手动登录。

下文保留初始渠道连接阶段的设计范围，当前收发行为以上述实现及 `chat-roadmap.md` 第 10 节为准。
