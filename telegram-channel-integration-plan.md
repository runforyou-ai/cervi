...
第一步在私聊文本事务提交后 best-effort 调用 `getUserProfilePhotos` 刷新渠道身份头像；失败不改变消息成功状态。版本变化时，服务端通过 `getFile` 下载经过大小和图片魔数校验的内容，并按企业当前配置写入本地目录或 S3 对象存储，再通过 `files` 生命周期激活为联系人头像；成员端只接收 Cervi 文件地址，任何响应、重定向和日志都不得暴露 Telegram 下载地址或 Token。头像删除会清空引用并回收旧文件，乱序消息不得覆盖更高 `message_id` 已确认的头像版本。
