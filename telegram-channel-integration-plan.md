...
## 当前实现：Telegram 私聊文本双向收发

本工作区已扩展到人工客服的私聊文本收发：入站复用客户会话，外发使用 `sendMessage` 和持久投递记录。Web、桌面收件箱统一使用时钟、对勾和提醒图标表示发送中、已发送和需要关注。本地提交、排队、平台调用和自动重试都归为发送中；失败、渠道停用和结果未确认通过提示说明具体原因，并提供人工重试、标记已发送和标记失败。
...
- Telegram 仍不开放 AI 自动回复、引用、媒体、群聊、频道或个人账号接入。Website 消息不创建外部投递。
